<template><div><h1 id="react源码架构" tabindex="-1"><a class="header-anchor" href="#react源码架构" aria-hidden="true">#</a> react源码架构</h1>
<h2 id="react15架构" tabindex="-1"><a class="header-anchor" href="#react15架构" aria-hidden="true">#</a> React15架构</h2>
<p>React15架构分为两层:</p>
<ul>
<li>reconciler(协调器)：负责找出组件变化</li>
<li>renderer(渲染器)：负责操作dom，将变化的地方渲染到页面</li>
</ul>
<h3 id="reconciler" tabindex="-1"><a class="header-anchor" href="#reconciler" aria-hidden="true">#</a> Reconciler</h3>
<p>reconciler主要做了这么几件事：</p>
<ul>
<li>执行函数式组件或者class组件的render方法，拿到其返回值的jsx生成虚拟DOM（虚拟DOM其实本质上就是js对象，其中包括了所有dom的属性，以及props，state）</li>
<li>通过深度优先遍历比较上次虚拟DOM与这次虚拟DOM的区别</li>
<li>通知renderer将变化的虚拟DOM渲染</li>
</ul>
<h3 id="renderer" tabindex="-1"><a class="header-anchor" href="#renderer" aria-hidden="true">#</a> Renderer</h3>
<p>有虚拟DOM的存在，就可以实现跨平台的功能，通过不同的渲染器，渲染在不同平台上。我们前端最熟悉的是负责在浏览器环境渲染的<strong>Renderer</strong> —— ReactDOM。</p>
<ul>
<li><code v-pre>ReactNative (opens new window)</code>渲染器，渲染<code v-pre>App</code>原生组件</li>
<li><code v-pre>ReactTest (opens new window)</code>渲染器，渲染出纯<code v-pre>Js</code>对象用于测试</li>
<li><code v-pre>ReactArt (opens new window)</code>渲染器，渲染到Canvas, <code v-pre>SVG </code>或<code v-pre> VML (IE8)</code></li>
</ul>
<h3 id="react15架构的缺点" tabindex="-1"><a class="header-anchor" href="#react15架构的缺点" aria-hidden="true">#</a> React15架构的缺点</h3>
<p>由于递归执行，所以更新一旦开始，中途就无法中断。当层级很深时，递归更新时间超过了16ms，用户交互就会卡顿。因此在React16版本提出了concurrent模式，该模式用可中断的异步更新替代同步更新。</p>
<h2 id="react16架构" tabindex="-1"><a class="header-anchor" href="#react16架构" aria-hidden="true">#</a> React16架构</h2>
<p>React16架构可以分为三层：</p>
<ul>
...
</span></code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>整个<strong>Scheduler</strong>与<strong>Reconciler</strong>的工作都在内存中进行。只有当所有组件都完成<strong>Reconciler</strong>的工作，才会统一交给<strong>Renderer</strong>。</p>
...
<p>首次执行<code v-pre>ReactDOM.render</code>会创建<code v-pre>fiberRootNode</code>（源码中叫<code v-pre>fiberRoot</code>）和<code v-pre>rootFiber</code>。其中<code v-pre>fiberRootNode</code>是整个应用的根节点，<code v-pre>rootFiber</code>是<code v-pre>&lt;App/&gt;</code>所在组件树的根节点。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/rootfiber.png" alt="rootFiber"></p>
...
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>React.createElement</code> 方法会返回一个包含组件数据的对象，对象有个参数<code v-pre>$$typeof: REACT_ELEMENT_TYPE</code>标记了该对象是个<code v-pre>React Element</code></p>
<p><code v-pre>JSX</code>是一种描述当前组件内容的数据结构，他不包含组件<strong>schedule</strong>、<strong>reconcile</strong>、<strong>render</strong>所需的相关信息。</p>
<p>比如如下信息就不包括在<code v-pre>JSX</code>中：</p>
<ul>
<li>组件在更新中的<code v-pre>优先级</code></li>
<li>组件的<code v-pre>state</code></li>
<li>组件被打上的用于<strong>Renderer</strong>的<code v-pre>标记</code></li>
</ul>
<p>所以，在组件<code v-pre>mount</code>时，<code v-pre>Reconciler</code>根据<code v-pre>JSX</code>描述的组件内容生成组件对应的<code v-pre>Fiber节点</code>。</p>
<p>在<code v-pre>update</code>时，<code v-pre>Reconciler</code>将<code v-pre>JSX</code>与<code v-pre>Fiber节点</code>保存的数据对比，生成组件对应的<code v-pre>Fiber节点</code>，并根据对比结果为<code v-pre>Fiber节点</code>打上<code v-pre>标记</code></p>
...
<span class="token keyword">function</span> <span class="token function">workLoopConcurrent</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>performConcurrentWorkOnRoot</code> 多了个判断浏览器帧是否有时间。<code v-pre>shouldYield</code>会终止循环，知道浏览器有空闲时间后再继续遍历。</p>
<p><code v-pre>performUnitOfWork</code> 方法会创建下一个Fiber节点，并将workInProgress与其子Fiber节点连接。<code v-pre>performUnitOfwork</code>分为两个过程，“递”和“归”</p>
<p><strong>递阶段:</strong></p>
<p>从<code v-pre>rootFiber</code>向下深度优先遍历，并调用<code v-pre>beginWork</code>方法。</p>
<p>该方法做的事情是为传入的Fiber节点创建其子Fiber节点，并将两个Fiber节点连接起来。</p>
<p>当遍历到叶子节点（即没有子组件的组件）时就会进入“归”阶段。</p>
<p><strong>归过程:</strong></p>
<p>在“归”阶段会调用<code v-pre>completeWork</code>处理<code v-pre>Fiber节点</code>。当某个<code v-pre>Fiber节点</code>执行完<code v-pre>completeWork</code>，如果其存在<code v-pre>兄弟Fiber节点</code>（即<code v-pre>fiber.sibling !== null</code>），会进入其<code v-pre>兄弟Fiber</code>的“递”阶段。</p>
<p>如果不存在<code v-pre>兄弟Fiber</code>，会进入<code v-pre>父级Fiber</code>的“归”阶段。</p>
<p>“递”和“归”阶段会交错执行直到“归”到<code v-pre>rootFiber</code>。至此，<code v-pre>render阶段</code>的工作就结束了。</p>
<h3 id="beginwork" tabindex="-1"><a class="header-anchor" href="#beginwork" aria-hidden="true">#</a> beginwork</h3>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220226153155497.png" alt="image-20220226153155497"></p>
<p><code v-pre>beginWork</code>的工作可以分为两部分：</p>
<ul>
...
<span class="token punctuation">)</span><span class="token operator">:</span> Fiber <span class="token operator">|</span> <span class="token keyword">null</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="diff算法" tabindex="-1"><a class="header-anchor" href="#diff算法" aria-hidden="true">#</a> diff算法</h4>
<p>diff算法需要判断新节点是不是单节点，单节点只有两种情况oldfiber链是否为空，如果为空那就新建一个节点，如果不为空，就找到和之前key相同的节点，删除其余节点</p>
<p>多节点则需要分为四种情况：节点更新，新增节点，删除节点，节点移动，多节点更新需要经过最多三轮的遍历（不过感觉是两轮的样子），每一轮都是上轮结束的断点的继续。</p>
<p>第一轮遍历会从头开始遍历newChildren，会逐个与oldFiber链中的节点进行比较，如果说key和tag都没有变化，那么就clone props更新的节点，props使用新的props，实现节点更新。</p>
<p>有变化则认为不是节点更新，直接进入下一轮循环。</p>
<p>我们称保留原位的节点为固定节点，第一轮遍历如果没有跳出循环的话就会设置一个lastPlaceIndex，用来记录最右边的固定节点。</p>
<p>旧： A - B - C - <code v-pre>D - E</code>
新： A - B - C</p>
<p>删除节点：当新节点遍历完之后如果oldFibers还没遍历完，就会删除后续没遍历完的节点。</p>
<p>旧： A - B - C
新： A - B - C - <code v-pre>D - E</code></p>
<p>新增节点：和删除节点类似，如果新节点没遍历完，那么新增后续节点。</p>
<p>旧 A - B - <code v-pre>C - D - E - F</code>
新 A - B - <code v-pre>D - C - E</code></p>
<p>节点移动：先将剩余oldFibers放入key为键，值为oldFiber节点的map中，成为existingChildren.</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果想要通知<code v-pre>Renderer</code>要操作Fiber对应的DOM节点，那么就需要你有fiber有对应的DOM节点以及effectTag上有对应的操作，但是在mount时<code v-pre>fiber.stateNode===null</code>,并且在<code v-pre>reconcileChildren</code>中也没有为其添加effectTag，那么首屏渲染该如何完成呢？</p>
<p>fiber.stateNode会在completeWork的时候创建，但mount时是不会给每个Fiber的<code v-pre>effectTag</code>赋值<code v-pre>placement</code>的，因为在commit阶段每次进行一次插入操作的效率是很低的，为了解决这个问题，在<code v-pre>mount</code>时只有<code v-pre>rootFiber</code>会赋值<code v-pre>Placement effectTag</code>，在<code v-pre>commit阶段</code>只会执行一次插入操作。</p>
<h3 id="completework" tabindex="-1"><a class="header-anchor" href="#completework" aria-hidden="true">#</a> completeWork</h3>
<h4 id="update" tabindex="-1"><a class="header-anchor" href="#update" aria-hidden="true">#</a> update</h4>
<p>update时Fiber节点已经存在对应DOM节点，因此只需要处理props，比如：</p>
<ul>
<li><code v-pre>onClick</code>、<code v-pre>onChange</code>等回调函数的注册</li>
<li>处理<code v-pre>style prop</code></li>
...
<ul>
<li>为<code v-pre>Fiber节点</code>生成对应的<code v-pre>DOM节点</code></li>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在归操作的过程中每次都会将生成自己的DOM节点，接着把子DOM节点插入刚生成的DOM节点中，这样在归操作结束后，我们就已经构建好了一个DOM树，这时只需要在rootFiber加上Placement effectTag就可以把整颗DOM树添加到页面中去了。</p>
<h4 id="effectlist" tabindex="-1"><a class="header-anchor" href="#effectlist" aria-hidden="true">#</a> effectList</h4>
<p>有一个问题：commit阶段的任务是修改的需要操作的Fiber节点对应的DOM节点，但是commit阶段需要再次遍历Fiber树查看其effectTag来判断需要如何更新吗，显然这样效率很低。</p>
<p>为了解决这个问题，在<code v-pre>completeWork</code>的上层函数<code v-pre>completeUnitOfWork</code>中，每个执行完<code v-pre>completeWork</code>且存在<code v-pre>effectTag</code>的<code v-pre>Fiber节点</code>会被保存在一条被称为<code v-pre>effectList</code>的单向链表中。</p>
<p>render阶段流程结束，将fiberRoot传入<code v-pre>commitRoot()</code>中，开始commit阶段</p>
<h2 id="commit阶段" tabindex="-1"><a class="header-anchor" href="#commit阶段" aria-hidden="true">#</a> commit阶段</h2>
<p><code v-pre>commit</code>阶段的主要工作（即<code v-pre>Renderer</code>的工作流程）分为三部分：</p>
<ul>
<li>before mutation阶段（执行<code v-pre>DOM</code>操作前）</li>
<li>mutation阶段（执行<code v-pre>DOM</code>操作）</li>
<li>layout阶段（执行<code v-pre>DOM</code>操作后）</li>
</ul>
<h1 id="合成事件" tabindex="-1"><a class="header-anchor" href="#合成事件" aria-hidden="true">#</a> 合成事件</h1>
<h2 id="为什么需要有合成事件" tabindex="-1"><a class="header-anchor" href="#为什么需要有合成事件" aria-hidden="true">#</a> 为什么需要有合成事件</h2>
<ul>
<li>
<p>进行浏览器兼容，实现更好的跨平台</p>
<p>React 采用的是顶层事件代理机制，能够保证冒泡一致性，可以跨浏览器执行。React 提供的合成事件用来抹平不同浏览器事件对象之间的差异，将不同平台事件模拟合成事件。</p>
</li>
<li>
<p>避免垃圾回收</p>
<p>事件对象可能会被频繁创建和回收，因此 React 引入<strong>事件池</strong>，在事件池中获取或释放事件对象。<strong>即 React 事件对象不会被释放掉，而是存放进一个数组中，当事件触发，就从这个数组中弹出，避免频繁地去创建和销毁(垃圾回收)</strong>。</p>
</li>
<li>
<p>方便事件统一管理和事务机制</p>
</li>
...
<th style="text-align:center">React 事件</th>
</tr>
</thead>
<tbody>
<tr>
<td>事件名称命名方式</td>
<td style="text-align:center">名称全部小写<br/>（onclick, onblur）</td>
<td style="text-align:center">名称采用小驼峰<br/>（onClick, onBlur）</td>
</tr>
<tr>
<td>事件处理函数语法</td>
...
<tr>
<td>阻止默认行为方式</td>
<td style="text-align:center">事件返回 <code v-pre>false</code></td>
<td style="text-align:center">使用 <code v-pre>e.preventDefault()</code> 方法</td>
</tr>
</tbody>
</table>
</li>
</ul>
<h2 id="合成事件触发流程" tabindex="-1"><a class="header-anchor" href="#合成事件触发流程" aria-hidden="true">#</a> 合成事件触发流程</h2>
<p>当真实 DOM 元素触发事件，会冒泡到 <code v-pre>document</code> 对象后，再处理 React 事件；所以会先执行原生事件，然后处理 React 事件；最后真正执行 <code v-pre>document</code> 上挂载的事件。采用的是事件委托。</p>
<h2 id="react事件池" tabindex="-1"><a class="header-anchor" href="#react事件池" aria-hidden="true">#</a> React事件池</h2>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个函数体可看成两部分：<strong>事件对象的合成和事件收集</strong> 、 <strong>事件执行</strong>，涵盖了上述三个过程。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>listeren就是遍历得到的事件处理函数，由于是同一个事件，listener会共享这个事件，比如传参的时候就会传入这个事件处理函数。</p>
<p>合成事件会将事件都绑定到root上，react17之前是document，在render阶段的completeWork阶段时会判断该prop是不是事件进行相应处理。在绑定过程中，会在root上绑定带有优先级的listerner，带有targetContainer,domEventName,listerner(这个listerner带有容器名和事件名)，在接下来触发事件的时候会有一个数组称他为执行路径，触发事件时，会从触发事件的那个元素开始一直往上收集绑定在fiber节点身上的事件，并push到执行路径里，事件收集完毕，事件开始执行，事件执行会根据是事件冒泡还是事件捕获来确定遍历顺序，每执行一个事件监听函数，就可以更改公用的合成事件上的currentTarget.</p>
<p>https://segmentfault.com/a/1190000039108951</p>
<h1 id="对函数式组件的理解" tabindex="-1"><a class="header-anchor" href="#对函数式组件的理解" aria-hidden="true">#</a> 对函数式组件的理解</h1>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>useEffect可以看作是<code v-pre>componentDidMount/componentDidUpdate/componentWillUnmount</code> 这三个生命周期函数的替代。但不是完全相同。</p>
<p>useEffect叫做副作用函数，因此他应该是放副作用函数的hook，函数式组件是一个纯函数，因此，在有副作用的时候我们就需要用到useEffect，因此我们不能把useEffect和<code v-pre>componentDidMount/componentDidUpdate/componentWillUnmount</code> 这三个生命周期函数等同起来。</p>
<p>useEffect是异步调用的，componenDidMount是同步调用的。</p>
<p>react源码分为三个部分：</p>
<p>调度器</p>
<p>协调器</p>
<p>渲染器</p>
<p>协调器会为需要更新的fiber打上标签，并生成一条effectList，渲染器根据effectList执行对应的操作。当渲染器遍历到该fiber,并且有passive标记，那么就会执行其useEffect</p>
<p>而协调器会从上往下一次遍历，再从下往上遍历，在从下往上的过程中会生成effectList，因此effectList的顺序是从下往上的，因此当一个组件创建之后，会从下往上调用useEffect</p>
<p>useEffect有两个参数，第一个参数传入一个函数，第二个参数传入一个依赖数组，当依赖数组中的内容变化时，（可以简单地理解为vue中的watch，即使好多人认为不能这么理解）将会执行第一个参数中的函数，第一个参数的return必须是一个函数，用于组件销毁时调用。</p>
<p>依赖数组为空和不传入第二个参数的区别，不传入第二个参数就会在每次组件创建和组件更新时调用，而依赖数组为空只会在组件刚创建的时候调用。</p>
<h3 id="源码分析useeffect和uselayouteffect" tabindex="-1"><a class="header-anchor" href="#源码分析useeffect和uselayouteffect" aria-hidden="true">#</a> 源码分析useEffect和useLayoutEffect</h3>
...
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>每个hook对象的next指向下一个生成的hook对象形成一个hooks链表，hooks链表最终会放到fiber节点的memoizedState属性上。</p>
<p>挂载时，组件上没有任何hooks的信息，所以，这个过程主要是在fiber上创建hooks链表。</p>
...
<p>useState放的就是state值，而use(Layout)Effect放的就是effect对象</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>effect对象除了会挂载到fiber.memoizedState上,还会保存在fiber的updateQueue,updateQueue链表是在completeWork阶段根据props的不同而创建的链表，将来如果这个节点需要更新，那么就会遍历这个fiber节点的updateQueue。updateQueue是一个环状链表，挂载或者更新时会把effect链表放到updateQueue后面</p>
<h4 id="流程概述" tabindex="-1"><a class="header-anchor" href="#流程概述" aria-hidden="true">#</a> 流程概述</h4>
<p>基于上面的数据结构，对于use（Layout）Effect来说，React做的事情就是</p>
<ul>
<li>render阶段：函数组件开始渲染的时候，创建出对应的hook链表挂载到workInProgress的memoizedState上，并创建effect链表，但是基于上次和本次依赖项的比较结果， 创建的effect是有差异的。这一点暂且可以理解为：依赖项有变化，effect可以被处理，否则不会被处理。</li>
<li>commit阶段：异步调度useEffect，layout阶段同步处理useLayoutEffect的effect。等到commit阶段完成，更新应用到页面上之后，开始处理useEffect产生的effect。</li>
</ul>
<p>第二点提到了一个重点，就是useEffect和useLayoutEffect的执行时机不一样，前者被异步调度，当页面渲染完成后再去执行，不会阻塞页面渲染。 后者是在commit阶段新的DOM准备完成，但还未渲染到屏幕之前，同步执行。</p>
<h4 id="实现细节" tabindex="-1"><a class="header-anchor" href="#实现细节" aria-hidden="true">#</a> 实现细节</h4>
<p>在commit阶段,会有三个地方调度useEffect：</p>
<ul>
<li>commit开始时：这和useEffect异步调度的特点有关，它以一般的优先级被调度，这就意味着一旦上一次更新有更高优先级的任务进入到commit阶段，上一次任务的useEffect就有可能没得到执行。所以在本次更新开始前，需要先将之前的useEffect都执行掉，以保证本次调度的useEffect都是本次更新产生的。</li>
<li>beforeMutation阶段：这个是实打实地针对effectList上有副作用的节点，去异步调度useEffect。在实现上利用scheduler的异步调度函数：<code v-pre>scheduleCallback</code>，将执行useEffect的动作作为一个任务去调度，这个任务会异步调用。</li>
<li>layout阶段layout阶段填充effect执行数组：真正useEffect执行的时候，实际上是先执行上一次effect的销毁，再执行本次effect的创建。React用两个数组来分别存储销毁函数和 创建函数，这两个数组的填充就是在layout阶段，到时候循环释放执行两个数组中的函数即可。</li>
</ul>
<p>在填充effect执行数组时，只把有HasEffect tag的才会被添加进执行数组，这是因为在生成effect对象的时候，会根据依赖项是否有变化，来确定tag，如果没有变化，那么tag就是hooksFlags(useEffect或者useLayoutEffect),如果变化了就是HasEffect tag。</p>
<h4 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h4>
<p>在每次执行useEffect函数时，都会创建一个effect对象，但是挂载和更新时调用的是两个函数，render阶段，在挂载时会直接创建一个hook对象，并且创建effect对象挂载到hook对象还有updateQueue上，在更新时会根据hook对象上的effect对比前后两次的依赖项，如果依赖项有变化tag就会加入HookHasEffect标志位，接着把effect挂载到upateQueue上。commit阶段，在commit开始时由于useEffect是一般优先级被调度，因此可能会被打断，因此要先把之前的useEffect执行掉，beforeMutation会异步调度effectList上有副作用的节点，layout阶段会根据tag是否有HasEffectTag判断是否加入执行数组，先循环destory函数，再循环create函数。useLayoutEffect的destory和create分别在mutation和layout阶段同步执行。</p>
<p>https://zhuanlan.zhihu.com/p/346696902</p>
...
<h1 id="错误边界" tabindex="-1"><a class="header-anchor" href="#错误边界" aria-hidden="true">#</a> 错误边界</h1>
