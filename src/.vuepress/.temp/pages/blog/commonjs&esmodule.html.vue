<template><div><h1 id="commonjs的出现" tabindex="-1"><a class="header-anchor" href="#commonjs的出现" aria-hidden="true">#</a> commonjs的出现</h1>
<p>在最开始网站的业务没那么复杂，js只是作为一门脚本语言，它不需要引入其他文件就可以解决已有业务，但随着业务需求越来越复杂，越来越需要模块化，commonjs就这样诞生了。再到后来es6把import，export加入了它们的关键字当中，也就有了现在的esmodule。首先这两个最大的不同之处在于：commonjs的module 和 require 只是对象和方法而已，而esmodule的import，export它们是关键字，es6新加的。</p>
<h1 id="commonjs和es6模块的差异" tabindex="-1"><a class="header-anchor" href="#commonjs和es6模块的差异" aria-hidden="true">#</a> commonjs和es6模块的差异</h1>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="compile的实现" tabindex="-1"><a class="header-anchor" href="#compile的实现" aria-hidden="true">#</a> _compile的实现</h3>
<p>首先我们思考,我们思考为什么可以在文件中使用<code v-pre>exports</code>, <code v-pre>require</code>, <code v-pre>module</code>, <code v-pre>__dirname</code>, <code v-pre>__filename</code>....这是因为我们在加载文件的时候，Mymodule.prototype. _compile把整个代码外面套了一个函数，函数里面传入了<code v-pre>exports</code>, <code v-pre>require</code>, <code v-pre>module</code>, <code v-pre>__dirname</code>, <code v-pre>__filename</code>,然后把这个函数执行一遍，就可以拿到exports了</p>
