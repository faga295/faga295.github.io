<template><div><h1 id="垃圾回收机制" tabindex="-1"><a class="header-anchor" href="#垃圾回收机制" aria-hidden="true">#</a> 垃圾回收机制</h1>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/1707342869-5c9358c332559.png" alt="image"></p>
<p>从根出发(全局变量)被引用的对象会被标记，没有被标记的对象都会被删除</p>
<h4 id="什么是垃圾" tabindex="-1"><a class="header-anchor" href="#什么是垃圾" aria-hidden="true">#</a> 什么是垃圾</h4>
<p>一般来说没有被引用的对象就是垃圾，就是要被清除， 有个例外如果几个对象引用形成一个环，互相引用，但根访问不到它们，这几个对象也是垃圾，也要被清除。</p>
<!--more--> 
<h4 id="如何检垃圾" tabindex="-1"><a class="header-anchor" href="#如何检垃圾" aria-hidden="true">#</a> 如何检垃圾</h4>
<p>一种算法是标记-清除算法,其策略是：</p>
<ul>
<li>
<p>JavaScript中的垃圾收集器运行时会给<strong>存储在内存中的所有变量</strong>都加上标记；</p>
</li>
<li>
<p>然后去掉环境中的变量以及被环境中的变量引用的变量的标记；</p>
</li>
<li>
<p>此后，再被加上标记的变量被视为准备删除的变量；</p>
</li>
<li>
<p>最后，垃圾收集器完成内存清除，销毁那些带标记的值并回收其占用的内存空间。</p>
</li>
</ul>
<p>局限：</p>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>由于a,b都被引用，因此a,b都不会被清除，如果频繁的调用改函数，则会造成很严重的内存泄漏。</p>
<h2 id="v8垃圾回收机制" tabindex="-1"><a class="header-anchor" href="#v8垃圾回收机制" aria-hidden="true">#</a> v8垃圾回收机制</h2>
<p>v8将堆内存分为了新生代和老生代，新生代属于是生命周期比较短的，而老生代属于是生命周期比较长的，对于这两种区域v8分配了两种不同的垃圾回收器：</p>
<ul>
<li>副垃圾回收器 - Scavenge：主要负责新生代的垃圾回收。</li>
<li>主垃圾回收器 - Mark-Sweep &amp; Mark-Compact：主要负责老生代的垃圾回收。</li>
</ul>
<h3 id="新生代垃圾回收器-scavenge" tabindex="-1"><a class="header-anchor" href="#新生代垃圾回收器-scavenge" aria-hidden="true">#</a> 新生代垃圾回收器 - Scavenge</h3>
<p>新生代垃圾回收器使用的是scavenge算法，一种用空间换时间的算法，对于占用空间不大的场景比较适用，新生代区域由于生命周期短因此给他分配的空间也小，使用scavenge算法非常合适。</p>
<p>scavenge算法做的事情就是把新生代中的对象标记为活动对象和非活动对象，将新生代区域分成两个部分一个是from-space,一个是to-space，还没进行垃圾回收前新生代对象都是在from-space中的。在标记完成之后，会将活动对象复制到to-space中去，并有序的排列起来。再把from-space和to-space角色互换，有点像fiber双缓存hhh</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200925123816388.png" alt=""></p>
<h4 id="新生代对象什么时候变成老生代" tabindex="-1"><a class="header-anchor" href="#新生代对象什么时候变成老生代" aria-hidden="true">#</a> 新生代对象什么时候变成老生代</h4>
<ol>
<li>当新生代的To space内存占满25%时，此时再从From space拷贝对象将不会再放入To空间中以防影响后续的新对象分配，而将其直接复制到老生代空间中。</li>
<li>在进行一次垃圾回收后，第二次GC时，发现已经经历过一次GC的对象在从From空间复制时直接复制到老生代。</li>
<li>在新对象分配时大部分对象被分配到新生代的From semispace，但当这个对象的体积过大，超过1MB的内存页时，直接分配到老生代中的large Object Space。</li>
</ol>
<h3 id="老生代垃圾回收-mark-sweep-mark-compact" tabindex="-1"><a class="header-anchor" href="#老生代垃圾回收-mark-sweep-mark-compact" aria-hidden="true">#</a> 老生代垃圾回收 - Mark-Sweep &amp; Mark-Compact</h3>
<p>老生代的垃圾回收就不能用scavenge算法了，上面说到过scavenge算法适用于空间占用不大的场景，因此在老生代垃圾回收中我们用了另一种算法：<code v-pre>Mark-Sweep</code>(标记清除)和<code v-pre>Mark-Compact</code>(标记整理) 算法。</p>
<h4 id="mark-sweep" tabindex="-1"><a class="header-anchor" href="#mark-sweep" aria-hidden="true">#</a> Mark-Sweep</h4>
<p>和scavenge算法一样，标记清除算法是先将活动对象标记，然后直接清除非活动对象</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200925163922575.png" alt=""></p>
<p>从图中可以看到清除后会有一些空，这些空留着显然很不合理，因为一旦有一些占用空间比较大的对象，这些空就用不上了，因此还需要<code v-pre>Mark-Compact</code>算法的帮助，这个算法就是在标记清除之后重新帮忙整理的，将所有的活动对象往一端移动，移动完成后，直接清理掉边界外的内存。这样老生代垃圾回收器就完成啦。</p>
<h3 id="全停顿" tabindex="-1"><a class="header-anchor" href="#全停顿" aria-hidden="true">#</a> 全停顿</h3>
<p>垃圾回收算法在执行前，需要将应用逻辑暂停，执行完垃圾回收后再执行应用逻辑，这种行为称为「全停顿」</p>
<p>如果全停顿时间过长就会造成页面卡顿。</p>
<p>为了解决全停顿问题，v8做了几点优化：<strong>增量标记</strong>、<strong>惰性清理</strong>、<strong>并发</strong>、<strong>并行</strong></p>
<h4 id="增量标记" tabindex="-1"><a class="header-anchor" href="#增量标记" aria-hidden="true">#</a> 增量标记</h4>
<p>增量标记就是把原先标记所有对象的任务分成了几个小任务，穿插在js应用逻辑之间，增量标记在堆大小达到一定量的时候启用，并在每达到一定量时进行标记。</p>
<p>增量标记就会造成之前标记过的对象，在执行js程序后，指向了新的对象，但由于标记清除(mark-sweep)算法是不会对其进行二次标记的，因此需要进行写屏障操作，写屏障不仅会记录’老-&gt;新‘指针，也记录了'黑-&gt;白'指针，如果一个对象在两个对象之间反复横跳，将会把有用的对象清除。写屏障记录了黑指向白，如果有黑指向白的情况会直接将白的对象染成灰的。</p>
<p>标记清除算法中会把已经标记过的对象记为’黑‘，正在标记的对象记为’灰‘(灰对象是用来重启时确定从哪里开始的)，还没标记的对象记为’白‘</p>
<h4 id="写屏障" tabindex="-1"><a class="header-anchor" href="#写屏障" aria-hidden="true">#</a> 写屏障</h4>
<p>如果有个指针指向新生代对象，这个指针又是老生代对象来的(比如说是老生代对象的一个value)，那这种情况标记的时候如果想要看他到底是不是活动对象，这种情况总不能去遍历个老生代对象吧，这太复杂了，v8在这里又做了一个优化，就是建立一个缓存，在有老生代指向新生代的时候，写入这个缓存列表，这样在标记的时候只需要看下那个表就可以了。</p>
<h4 id="惰性清理" tabindex="-1"><a class="header-anchor" href="#惰性清理" aria-hidden="true">#</a> 惰性清理</h4>
<p>假如当前的可用内存足以让我们快速的执行代码，其实我们是没必要立即清理内存的，可以将清理的过程延迟一下，让JavaScript逻辑代码先执行，也无需一次性清理完所有非活动对象内存，垃圾回收器会按需逐一进行清理，直到所有的页都清理完毕。</p>
<h3 id="并发" tabindex="-1"><a class="header-anchor" href="#并发" aria-hidden="true">#</a> 并发</h3>
<p>并发式GC允许在在垃圾回收的同时不需要将主线程挂起，两者可以同时进行，只有在个别时候需要短暂停下来让垃圾回收器做一些特殊的操作。但是这种方式也要面对增量回收的问题，就是在垃圾回收过程中，由于JavaScript代码在执行，堆中的对象的引用关系随时可能会变化，所以也要进行<code v-pre>写屏障</code>操作。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200926003554103.png" alt=""></p>
<h3 id="并行" tabindex="-1"><a class="header-anchor" href="#并行" aria-hidden="true">#</a> 并行</h3>
<p>并行式GC允许主线程和辅助线程同时执行同样的GC工作，这样可以让辅助线程来分担主线程的GC工作，使得垃圾回收所耗费的时间等于总时间除以参与的线程数量（加上一些同步开销）。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200926004058072.png" alt=""></p>
<h3 id="目前v8的垃圾回收机制" tabindex="-1"><a class="header-anchor" href="#目前v8的垃圾回收机制" aria-hidden="true">#</a> 目前v8的垃圾回收机制</h3>
<p>在新生代垃圾回收中，采用并行机制，辅助线程做的事情是复制阶段即从<code v-pre>from-space</code>到<code v-pre>space-to</code>的过程，由于多个线程竞争一个新生代的堆的内存资源，可能出现有某个活动对象被多个线程进行复制操作的问题，为了解决这个问题，V8在第一个线程对活动对象进行复制并且复制完成后，都必须去维护复制这个活动对象后的指针转发地址，以便于其他协助线程可以找到该活动对象后可以判断该活动对象是否已被复制。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200926103100834.png" alt=""></p>
<p>老生代垃圾回收中，采用的是并发机制，在堆内存空间达到一定大小时会启用并发标记，因此也会启用写屏障机制，当并发标记完成或者动态分配的内存到达极限的时候，主线程会执行最终的快速标记步骤，这个时候主线程会挂起，主线程会再一次的扫描根集以确保所有的对象都完成了标记，确认完成之后，某些辅助线程会进行清理内存操作，某些辅助进程会进行内存整理操作。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20200926105712369.png" alt=""></p>
<h1 id="v8引擎" tabindex="-1"><a class="header-anchor" href="#v8引擎" aria-hidden="true">#</a> v8引擎</h1>
...
<li>TurboFan：compiler，即编译器，利用Ignitio所收集的信息，将Bytecode转换为优化的汇编代码；</li>
<li>Orinoco：garbage collector，<a href="https://blog.fundebug.com/2019/07/03/javascript-garbage-collection/" target="_blank" rel="noopener noreferrer">垃圾回收<ExternalLinkIcon/></a>模块，负责将程序不再需要的内存空间回收；</li>
</ul>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/2019-07-16-ignition-turbofan-pipeline.png" alt=""></p>
<p>Bytecode可以通过编译器到Machine Code,图中画的不太对</p>
<p>这里出现了<code v-pre>Bytecode</code>和<code v-pre>Machine Code</code> (实际上是汇编代码)，Bytecode类似于Machine Code，不过它没有对应的cpu，不需要为每一种cpu都生成不同的代码，引入Bytecode作为中间层，可以简化v8的编译流程。在执行字节码的过程中如果某段代码被执行了多次，就会将其认定为热点代码，编译器就会把热点代码编译为更高效的机器码，这些机器码是对字节码进行优化之后生成的。</p>
<h2 id="turbofan-编译器" tabindex="-1"><a class="header-anchor" href="#turbofan-编译器" aria-hidden="true">#</a> TurboFan 编译器</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="script标签中的async和defer属性" tabindex="-1"><a class="header-anchor" href="#script标签中的async和defer属性" aria-hidden="true">#</a> script标签中的async和defer属性</h1>
<h2 id="script" tabindex="-1"><a class="header-anchor" href="#script" aria-hidden="true">#</a> script</h2>
<p>浏览器在解析 HTML 的时候，如果遇到一个没有任何属性的 script 标签，就会暂停解析，先发送网络请求获取该 JS 脚本的代码内容，然后让 JS 引擎执行该代码，当代码执行完毕后恢复解析。可以看到，script 阻塞了浏览器对 HTML 的解析，如果获取 JS 脚本的网络请求迟迟得不到响应，或者 JS 脚本执行时间过长，都会导致白屏，用户看不到页面内容。</p>
<h2 id="async-script" tabindex="-1"><a class="header-anchor" href="#async-script" aria-hidden="true">#</a> async script</h2>
<p>async表示异步，当浏览器遇到带有 async 属性的 script 时，请求该脚本的网络请求是异步的，不会阻塞浏览器解析 HTML，一旦网络请求回来之后，如果此时 HTML 还没有解析完，浏览器会暂停解析，先让 JS 引擎执行代码，执行完毕后再进行解析。当然，如果在 JS 脚本请求回来之前，HTML 已经解析完毕了，那就啥事没有，立即执行 JS 代码，所以 async 是不可控的，因为执行时间不确定，你如果在异步 JS 脚本中获取某个 DOM 元素，有可能获取到也有可能获取不到。而且如果存在多个 async 的时候，它们之间的执行顺序也不确定，完全依赖于网络传输结果，谁先到执行谁。</p>
<h2 id="defer-script" tabindex="-1"><a class="header-anchor" href="#defer-script" aria-hidden="true">#</a> defer script</h2>
<p>defer 表示延迟，当浏览器遇到带有 defer 属性的 script 时，获取该脚本的网络请求也是异步的，不会阻塞浏览器解析 HTML，一旦网络请求回来之后，如果此时 HTML 还没有解析完，浏览器不会暂停解析并执行 JS 代码，而是等待 HTML 解析完毕再执行 JS 代码，如果存在多个 defer script 标签，浏览器（IE9及以下除外）会保证它们按照在 HTML 中出现的顺序执行，不会破坏 JS 脚本之间的依赖关系。</p>
<table>
<thead>
<tr>
<th style="text-align:center">script标签</th>
<th style="text-align:center">js执行顺序</th>
<th style="text-align:center">是否阻断解析html</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:center">script</td>
<td style="text-align:center">在html中的顺序</td>
<td style="text-align:center">阻塞</td>
</tr>
<tr>
<td style="text-align:center">script async</td>
<td style="text-align:center">网络请求返回顺序</td>
<td style="text-align:center">可能阻塞，也可能不阻塞</td>
</tr>
<tr>
<td style="text-align:center">script defer</td>
<td style="text-align:center">在html中的顺序</td>
<td style="text-align:center">不阻塞</td>
</tr>
</tbody>
</table>
<h1 id="浏览器渲染流程" tabindex="-1"><a class="header-anchor" href="#浏览器渲染流程" aria-hidden="true">#</a> 浏览器渲染流程</h1>
<p>从网络传给渲染引擎的 HTML 文件字节流是无法直接被渲染引擎理解的，所以要将其转化为渲染引擎能够理解的内部结构，这个结构就是 DOM。DOM 提供了对 HTML 文档结构化的表述。</p>
<h2 id="dom树如何生成" tabindex="-1"><a class="header-anchor" href="#dom树如何生成" aria-hidden="true">#</a> DOM树如何生成</h2>
<p>渲染引擎内部，有一个叫HTML 解析器<code v-pre>（HTMLParser）</code>的模块，它的职责就是负责将HTML 字节流转换为 DOM 结构。HTML解析器会随着HTML文档边加载边解析。具体流程是：</p>
<p>网络进程收到响应头后，看到<code v-pre>content-type</code>为<code v-pre>text/html</code>时,就会为请求创建一个渲染进程，渲染进程准备好后网络进程和渲染进程之间就会建立一个共享数据的通道，网络进程接收到数据后就往这个管道里面放，而渲染进程则从管道的另外一端不断地读取数据，并同时将读取的数据“喂”给 HTML 解析器.</p>
<p><strong>字节流转换为DOM</strong></p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/8946fb176bc4f3ff9c160d1d04dd03dc.png" alt=""></p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/709961d9786c7e32781a37f3f1321f05.png" alt=""></p>
...
&lt;/body>
&lt;/html>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>首先不可避免的是要下载，由于在执行<code v-pre>js</code>脚本之前，渲染引擎是不知道该脚本是否会操纵<code v-pre>CSSOM</code>，不管该脚本是否操纵了 <code v-pre>CSSOM</code>，都会执行<code v-pre>CSS </code>文件下载，解析操作，再执行 JavaScript 脚本。</p>
<p><em>和 HTML 一样，渲染引擎也是无法直接理解 <code v-pre>CSS</code> 文件内容的，所以需要将其解析成渲染引</em>
擎能够理解的结构，这个结构就是 <code v-pre>CSSOM</code>。等 DOM 和 <code v-pre>CSSOM </code>都构建好之后，渲染引擎就会构造布局树</p>
<p>**总结：**CSS不会阻塞DOM树的解析，但是会影响 JavaScript的运行，</p>
<p>JavaScript 会阻止DOM树的解析，</p>
<p>最终<code v-pre>CSS</code>（<code v-pre>CSSOM</code>）会影响DOM树的渲染，也可以说最终会影响布局树的生成</p>
<p><em><code v-pre>CSS</code>不会阻塞DOM解析，但是会阻塞DOM渲染 ,<code v-pre>JS</code>会阻塞DOM解析，<code v-pre>CSS</code>会阻塞<code v-pre>JS</code>的执行</em></p>
<p>**DOM树和CSSOM树是互斥的还是同时的？**同时的</p>
<p>**JS脚本阻塞DOM构建，js脚本会不会对CSSOM树影响？**不会阻塞CSSOM树</p>
<h2 id="重绘与回流" tabindex="-1"><a class="header-anchor" href="#重绘与回流" aria-hidden="true">#</a> 重绘与回流</h2>
<p>浏览器生成render树后会触发回流和重绘，回流获取节点的几何位置，重绘获取节点的绝对像素，接着会把这些信息发送给GPU，展示在页面上</p>
<h2 id="回流" tabindex="-1"><a class="header-anchor" href="#回流" aria-hidden="true">#</a> 回流</h2>
...
<p>回流一定触发重绘，重绘不一定会回流</p>
<h3 id="浏览器优化机制" tabindex="-1"><a class="header-anchor" href="#浏览器优化机制" aria-hidden="true">#</a> 浏览器优化机制</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="浏览器进程" tabindex="-1"><a class="header-anchor" href="#浏览器进程" aria-hidden="true">#</a> 浏览器进程</h1>
<ol>
...
<h2 id="渲染进程" tabindex="-1"><a class="header-anchor" href="#渲染进程" aria-hidden="true">#</a> 渲染进程</h2>
...
<li>当处理一些不能立即执行的代码时，会将对应的任务在其可以触发的时机，添加到事件队列的末端</li>
<li>事件循环机制会在JS引擎线程空闲时，循环访问事件队列的头部，如果有函数，则会将该函数推到执行栈中并立即执行</li>
</ul>
<p><strong>定时触发器线程</strong></p>
<ul>
<li>setInterval与setTimeout所在的线程</li>
<li>定时任务并不是由JS引擎计时的，是由定时触发线程来计时的</li>
<li>计时完毕后，将回调事件放入到事件队列中</li>
</ul>
<p><strong>异步http请求线程</strong></p>
<ul>
<li>浏览器有一个单独的线程用于处理AJAX请求</li>
<li>当请求完成时，若有回调函数，将回调事件放入到事件队列中</li>
</ul>
<p>异步http请求线程和定时器触发器线程都是通过js引擎线程通知的，在异步http请求线程请求完成后或者定时器等待一段时间后会将回调事件放入事件队列，事件队列是由事件触发线程管理的</p>
<h1 id="js-数据类型" tabindex="-1"><a class="header-anchor" href="#js-数据类型" aria-hidden="true">#</a> js 数据类型</h1>
<p><a href="https://blog.csdn.net/jiangjuanjaun/article/details/80327342" target="_blank" rel="noopener noreferrer">https://blog.csdn.net/jiangjuanjaun/article/details/80327342<ExternalLinkIcon/></a></p>
<h1 id="http-缓存" tabindex="-1"><a class="header-anchor" href="#http-缓存" aria-hidden="true">#</a> http 缓存</h1>
<p>http缓存分为强制缓存和协商缓存，强制缓存就是在响应头里告诉你以后多久都可以直接拿缓存了（Expires，Cache-Control）</p>
<p>协商缓存就是判断是不是更新了，如果没更新就可以直接拿缓存了。(Etag&amp;If-None-Match,Last-Modified&amp;If-Modified-Since)</p>
<h3 id="强制缓存" tabindex="-1"><a class="header-anchor" href="#强制缓存" aria-hidden="true">#</a> 强制缓存</h3>
<h4 id="expires头-兼容" tabindex="-1"><a class="header-anchor" href="#expires头-兼容" aria-hidden="true">#</a> Expires头（兼容）</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="协商缓存和强制缓存的优先级" tabindex="-1"><a class="header-anchor" href="#协商缓存和强制缓存的优先级" aria-hidden="true">#</a> 协商缓存和强制缓存的优先级</h3>
<p><strong>先判断强制缓存，如果强制缓存生效，直接使用缓存；如果强制缓存失效，再发请求跟服务器协商，看要不要使用缓存</strong>。</p>
<p>https://segmentfault.com/a/1190000038562294</p>
<h2 id="浏览器缓存的位置" tabindex="-1"><a class="header-anchor" href="#浏览器缓存的位置" aria-hidden="true">#</a> 浏览器缓存的位置</h2>
...
<p>是运行在浏览器背后的独立线程,要求请求必须是https，因为 Service Worker 中涉及到请求拦截，所以必须使用 HTTPS 协议来保障安全</p>
<h3 id="memory-cache" tabindex="-1"><a class="header-anchor" href="#memory-cache" aria-hidden="true">#</a> Memory Cache</h3>
<p>内存中的缓存，主要包含的是当前中页面中已经抓取到的资源，例如页面上已经下载的样式、脚本、图片等。内存缓存虽然读取高效，可是缓存持续性很短，会随着进程的释放而释放。一旦我们关闭 Tab 页面，内存中的缓存也就被释放了。</p>
<h3 id="disk-cache" tabindex="-1"><a class="header-anchor" href="#disk-cache" aria-hidden="true">#</a> Disk Cache</h3>
<p>存在磁盘内，容量大，读取速度慢，它会根据 HTTP Herder 中的字段判断哪些资源需要缓存，哪些资源可以不请求直接使用，哪些资源已经过期需要重新请求。</p>
<h4 id="prefetch-cache" tabindex="-1"><a class="header-anchor" href="#prefetch-cache" aria-hidden="true">#</a> prefetch Cache</h4>
...
4.支持扩展。ws协议定义了扩展，用户可以扩展协议，或者实现自定义的子协议。（比如支持自定义压缩算法等）</p>
<h2 id="其他特点" tabindex="-1"><a class="header-anchor" href="#其他特点" aria-hidden="true">#</a> 其他特点</h2>
...
<h2 id="如何建立连接" tabindex="-1"><a class="header-anchor" href="#如何建立连接" aria-hidden="true">#</a> 如何建立连接</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>重点请求首部意义如下：</p>
<p>Connection: Upgrade：表示要升级协议
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="数据帧格式" tabindex="-1"><a class="header-anchor" href="#数据帧格式" aria-hidden="true">#</a> 数据帧格式</h2>
...
本节的重点，就是讲解数据帧的格式。详细定义可参考 RFC6455 5.2节 。</p>
<p>1、数据帧格式概览
...
<p>%x0：表示一个延续帧。当Opcode为0时，表示本次数据传输采用了数据分片，当前收到的数据帧为其中一个数据分片。
%x1：表示这是一个文本帧（frame）
%x2：表示这是一个二进制帧（frame）
...
<h2 id="数据传递" tabindex="-1"><a class="header-anchor" href="#数据传递" aria-hidden="true">#</a> 数据传递</h2>
<p>一旦WebSocket客户端、服务端建立连接后，后续的操作都是基于数据帧的传递。</p>
<p>WebSocket根据opcode来区分操作的类型。比如0x8表示断开连接，0x0-0x2表示数据交互。</p>
<p>1、数据分片
WebSocket的每条消息可能被切分成多个数据帧。当WebSocket的接收方收到一个数据帧时，会根据FIN的值来判断，是否已经收到消息的最后一个数据帧。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="http" tabindex="-1"><a class="header-anchor" href="#http" aria-hidden="true">#</a> HTTP</h1>
<h2 id="request" tabindex="-1"><a class="header-anchor" href="#request" aria-hidden="true">#</a> request</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第一部分：请求行，用来说明请求类型,要访问的资源以及所使用的HTTP版本.</p>
...
<p>从第二行起为请求头部，HOST将指出请求的目的地.User-Agent,服务器端和客户端脚本都能访问它,它是浏览器类型检测逻辑的重要基础.该信息由你的浏览器来定义,并且在每个请求中自动发送等等，User-Agent由你的浏览器来定义，为了给用户更好的体验，例如pc端和移动端是不一样的，浏览器的UA字串的标准格式：浏览器标识 (操作系统标识; 加密等级标识; 浏览器语言) 渲染引擎标识版本信息</p>
<p>第三部分：空行，请求头部后面的空行是必须的</p>
<p>即使第四部分的请求数据为空，也必须有空行。</p>
<p>第四部分：请求数据也叫主体，可以添加任意的其他数据。
这个例子的请求数据为空。</p>
<h2 id="response" tabindex="-1"><a class="header-anchor" href="#response" aria-hidden="true">#</a> response</h2>
<p>HTTP 响应与 HTTP 请求相似，HTTP响应也由3个部分构成，分别是：</p>
<p>状态行
响应头(Response Header)
响应正文
状态行由协议版本、数字形式的状态代码、及相应的状态描述，各元素之间以空格分隔。</p>
<h4 id="常见的状态码和状态文本" tabindex="-1"><a class="header-anchor" href="#常见的状态码和状态文本" aria-hidden="true">#</a> 常见的状态码和状态文本</h4>
<ul>
<li>1 表示临时响应，只包含状态行和部分响应头信息，并以空行结束</li>
<li>2 表示成功被服务器接收、理解、并接受</li>
<li>3 表示要完成请求，需要进一步操作。 通常，这些状态代码用来重定向</li>
<li>4 表示请求错误</li>
<li>5 表示服务器错误</li>
</ul>
<p>100 表示客户端的部分请求已被服务器接收 ，客户端应当继续发送请求的剩余部分，或者如果请求已经完成，忽略这个响应</p>
<p>101：更换协议，比如<code v-pre>websocket</code>和<code v-pre>http2</code></p>
<p>200 OK 客户端请求成功</p>
<p>201 (已创建) 请求成功并且服务器创建了新的资源</p>
<p>202（已创建）服务器已经接收请求，但尚未处理</p>
<p>204 （无内容）服务器成功处理请求，但没有返回任何内容</p>
<p>206（部分内容）应用于 HTTP 分块下载或断点续传，表示响应返回的 body 数据并不是资源的全部，而是其中的一部分，也是服务器处理成功的状态。</p>
<p>301 Moved Permanently 永久移动 请求的网页已永久移动到新位置。 服务器返回此响应（对 GET 或 HEAD 请求的响应）时，会自动将请求者转到新位置
302 Moved Temporarily 临时移动 服务器目前从不同位置的网页响应请求，但请求者应继续使用原有位置来进行以后的请求
304 Not Modified 文件未修改，可以直接使用缓存的文件。</p>
<p>307 （临时重定向） 服务器目前从不同位置的网页响应请求，但请求者应继续使用原有位置来进行以后的请求</p>
<p>302 重定向后可能会将请求从post变为get，而307不会</p>
<p>301 302 303 307 是在请求头的location字段中表明重定向的地址</p>
<p>301 是永久重定向有利于seo，302在规范中是不允许修改请求方式的，但由于浏览器厂商并没有遵循这个规范，它会在302之后用get方式访问重定向地址。在浏览器厂商的这种操作下HTTP1.1推出了303和307。303是重定向后使用get请求访问资源，307是重定向后用原请求方式访问资源。</p>
<p>301有个重要特性就是会缓存(因为是永久重定向，可以通过cache-control:no-cache禁用缓存)，但是307不会</p>
<p>400 Bad Request 由于客户端请求有语法错误，不能被服务器所理解。
...
<p>get是安全幂等的，post会修改服务器上的资源，因此是不安全的，结果一不一定是相同的</p>
<h2 id="http1-1" tabindex="-1"><a class="header-anchor" href="#http1-1" aria-hidden="true">#</a> http1.1</h2>
...
<p>使用长连接之后，客户端、服务端怎么知道本次传输结束呢？两部分：1. 判断传输数据是否达到了Content-Length 指示的大小；2. 动态生成的文件没有 Content-Length ，它是分块传输（chunked），这时候就要根据 chunked 编码来判断，chunked 编码的数据在最后有一个空 chunked 块，表明本次传输数据结束，详见这里。什么是 chunked 分块传输呢？下面我们就来介绍一下。</p>
<h3 id="管道传输" tabindex="-1"><a class="header-anchor" href="#管道传输" aria-hidden="true">#</a> 管道传输</h3>
<p>即可在同一个 TCP 连接里面，客户端可以发起多个请求，只要第一个请求发出去了，不必等其回来，</p>
<p>就可以发第二个请求出去，可以<strong>减少整体的响应时间。</strong></p>
<p>举例来说，客户端需要请求两个资源。以前的做法是，在同一个TCP连接里面，先发送 A 请求，然后等</p>
<p>待服务器做出回应，收到后再发出 B 请求。管道机制则是允许浏览器同时发出 A 请求和 B 请求。</p>
<p>但是服务器还是按照<strong>顺序</strong>，先回应 A 请求，完成后再回应 B 请求。要是前面的回应特别慢，后面就会</p>
<p>有许多请求排队等着。这称为「队头堵塞」。</p>
<h3 id="队头阻塞" tabindex="-1"><a class="header-anchor" href="#队头阻塞" aria-hidden="true">#</a> 队头阻塞</h3>
<p>因为当顺序发送的请求序列中的一个请求因为某种原因被阻塞时，在后面排队的所有请求也一同被阻塞</p>
<p>了，会招致客户端一直请求不到数据，这也就是「<strong>队头阻塞</strong>」。</p>
<h3 id="缓存处理" tabindex="-1"><a class="header-anchor" href="#缓存处理" aria-hidden="true">#</a> 缓存处理</h3>
...
<h3 id="range头域" tabindex="-1"><a class="header-anchor" href="#range头域" aria-hidden="true">#</a> range头域</h3>
<p>HTTP1.0一些浪费带宽的现象，例如客户端只是需要某个对象的一部分，而服务器却将整个对象送过来了，并且不支持断点续传功能，HTTP1.1则在请求头引入了range头域，它允许只请求资源的某个部分，即返回码是206（Partial Content），这样就方便了开发者自由的选择以便于充分利用带宽和连接。</p>
<h3 id="新增了24个错误状态响应码" tabindex="-1"><a class="header-anchor" href="#新增了24个错误状态响应码" aria-hidden="true">#</a> 新增了24个错误状态响应码</h3>
<p>如409（Conflict）表示请求的资源与资源的当前状态发生冲突；410（Gone）表示服务器上的某个资源被永久性的删除。</p>
<h2 id="transfer-encoding" tabindex="-1"><a class="header-anchor" href="#transfer-encoding" aria-hidden="true">#</a> Transfer-Encoding</h2>
...
<p>chunked 和 multipart 两个名词在意义上有类似的地方，不过在 HTTP 协议当中这两个概念则不是一个类别的。multipart 是一种 Content-Type，标示 HTTP 报文内容的类型，而 chunked 是一种传输格式，标示报头将以何种方式进行传输。
chunked 传输不能事先知道内容的长度，只能靠最后的空 chunk 块来判断，因此对于下载请</p>
...
<p>domain:默认设置为发送请求的那个主域名 path表示主机下的哪些路径可以接受cookie domain和path共同指定cookie应该发送给哪些URL。</p>
<p>expire time 过期时间 secure cookie只能用https传输 httponly js不能访问cookie</p>
...
<p>**缺点：**对服务器性能有一定影响</p>
<h2 id="跨站攻击" tabindex="-1"><a class="header-anchor" href="#跨站攻击" aria-hidden="true">#</a> 跨站攻击</h2>
<h3 id="csrf" tabindex="-1"><a class="header-anchor" href="#csrf" aria-hidden="true">#</a> CSRF</h3>
...
<p><code v-pre>http://example.com/bbs/create_post.php?title=标题&amp;content=内容</code>
那么，我们只需要在论坛中发一帖，包含一链接：</p>
<p><code v-pre>http://example.com/bbs/create_post.php?title=我是脑残&amp;content=哈哈</code>
只要有用户点击了这个链接，那么他们的帐户就会在不知情的情况下发布了这一帖子。可能这只是个恶作剧，但是既然发贴的请求可以伪造，那么删帖、转帐、改密码、发邮件全都可以伪造。</p>
<p><strong>携带cookie的规则是根据请求的地址来判断，跟页面的地址无关。</strong></p>
<p>如何防范 CSRF 攻击？可以注意以下几点：</p>
<p>关键操作只接受 POST 请求</p>
<p>验证码</p>
...
<p>检测 Referer</p>
<p>常见的互联网页面与页面之间是存在联系的，比如你在 www.baidu.com 应该是找不到通往www.google.com 的链接的，再比如你在论坛留言，那么不管你留言后重定向到哪里去了，之前的那个网址一定会包含留言的输入框，这个之前的网址就会保留在新页面头文件的 Referer 中</p>
<p>通过检查 Referer 的值，我们就可以判断这个请求是合法的还是非法的，但是问题出在服务器不是任何时候都能接受到 Referer 的值，所以 Referer Check 一般用于监控 CSRF 攻击的发生，而不用来抵御攻击。</p>
...
<p>Token 使用原则</p>
<p>Token 要足够随机————只有这样才算不可预测
Token 是一次性的，即每次请求成功后要更新Token————这样可以增加攻击难度，增加预测难度
Token 要注意保密性————敏感操作使用 post，防止 Token 出现在 URL 中
注意：过滤用户输入的内容不能阻挡 csrf，我们需要做的是过滤请求的来源。</p>
<p>CSRF通过外域发送请求，浏览器会自动带上cookie，伪造成用户自己发送的请求。</p>
<h4 id="策略" tabindex="-1"><a class="header-anchor" href="#策略" aria-hidden="true">#</a> 策略</h4>
<p>阻止不明外域的访问：</p>
<ul>
<li>同源检测</li>
...
<p><code v-pre>&lt;script&gt;window.location.href=”http://www.baidu.com”;&lt;/script&gt;</code>
经过 escape 之后就成了：</p>
<p><code v-pre>&amp;lt;script&amp;gt;window.location.href=&amp;quot;http://www.baidu.com&amp;quot;&amp;lt;/script&amp;gt;</code>
它现在会像普通文本一样显示出来，变得无毒无害，不能执行了。</p>
...
<p>存储型xss是通过请求将数据加入数据库，这些数据会展示在页面上，如留言，评论</p>
<h4 id="反射型" tabindex="-1"><a class="header-anchor" href="#反射型" aria-hidden="true">#</a> 反射型</h4>
<p>反射型xss是攻击者构造恶意url，当用户访问这个url时，会将url中的数据拼接在html中，当用户点开url就会执行恶意脚本。</p>
<h4 id="dom型" tabindex="-1"><a class="header-anchor" href="#dom型" aria-hidden="true">#</a> dom型</h4>
<p>dom主要是因为前端JavaScript自身的漏洞，攻击者构造恶意url，前端javascript取出url中的数据，并执行恶意脚本</p>
<h4 id="xss预防" tabindex="-1"><a class="header-anchor" href="#xss预防" aria-hidden="true">#</a> xss预防</h4>
<h5 id="预防存储型和反射型" tabindex="-1"><a class="header-anchor" href="#预防存储型和反射型" aria-hidden="true">#</a> 预防存储型和反射型</h5>
<p>这两种都是服务端取出恶意代码拼接到html上，主要有两种方法：</p>
<p>1.纯前端渲染</p>
<p>2.对html做充分转义</p>
<p>纯前端渲染就是我们现在常用的，通过ajax发送数据，再把数据渲染在页面上。</p>
<p>如果html拼接是有必要的，那么需要对html做充分的转义，做好充分的html转义是非常复杂的。</p>
<h5 id="预防dom型" tabindex="-1"><a class="header-anchor" href="#预防dom型" aria-hidden="true">#</a> 预防dom型</h5>
<p>尽量不要使用innterHTML,outerHTML,domcument.write，如果是vue或者react，尽量不要使用v-html，dangerouslySetInnerHTML，onclick，location，a的href属性都是可以执行JavaScript代码的，不要将不信任的数据传递给这些api</p>
<h5 id="其他防范措施" tabindex="-1"><a class="header-anchor" href="#其他防范措施" aria-hidden="true">#</a> 其他防范措施</h5>
<p>csp，可以防止加载外域脚本；</p>
<p>限制输入长度</p>
<p>http-only，无法通过js脚本获取到cookie</p>
<p>验证码</p>
<h2 id="http2-0" tabindex="-1"><a class="header-anchor" href="#http2-0" aria-hidden="true">#</a> http2.0</h2>
<p>http2.0是一种安全高效的下一代http传输协议。安全是因为http2.0建立在https协议的基础上，高效是因为它是通过二进制分帧来进行数据传输。</p>
...
<p>流是连接中的一个虚拟信道，可以承载双向消息传输，可以承载一条或多条消息。每个流有唯一整数标识符。为了防止两端流ID冲突，客户端发起的流具有奇数ID，服务器端发起的流具有偶数ID。</p>
...
<p><img src="https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/10/31/16e208ee1d0caab8~tplv-t2oaga2asx-watermark.awebp" alt="img"></p>
<h3 id="多路复用-连接共享" tabindex="-1"><a class="header-anchor" href="#多路复用-连接共享" aria-hidden="true">#</a> 多路复用/连接共享</h3>
...
<p>http 2.0 连接都是持久化的，而且客户端与服务器之间也只需要一个连接（每个域名一个连接）即可。http2连接可以承载数十或数百个流的复用，<strong>多路复用意味着来自很多流的数据包能够混合在一起通过同样连接传输</strong>。<strong>当到达终点时，再根据不同帧首部的流标识符重新连接将不同的数据流进行组装。</strong></p>
<p><img src="https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/10/31/16e208ee1cebfa44~tplv-t2oaga2asx-watermark.awebp" alt="img"></p>
<p><img src="https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/10/31/16e208ee1e5c87aa~tplv-t2oaga2asx-watermark.awebp" alt="img"></p>
<h3 id="头部压缩" tabindex="-1"><a class="header-anchor" href="#头部压缩" aria-hidden="true">#</a> 头部压缩</h3>
...
<h4 id="压缩原理" tabindex="-1"><a class="header-anchor" href="#压缩原理" aria-hidden="true">#</a> 压缩原理</h4>
<p>用header字段表里的索引代替实际的header。</p>
<p>http/2的HPACK算法使用一份索引表来定义常用的http Header，把常用的 http Header 存放在表里，请求的时候便只需要发送在表里的索引位置即可。</p>
<p><img src="https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/10/31/16e208ee3d18e3b3~tplv-t2oaga2asx-watermark.awebp" alt="img"></p>
...
<h3 id="请求优先级" tabindex="-1"><a class="header-anchor" href="#请求优先级" aria-hidden="true">#</a> 请求优先级</h3>
...
<p>●优先级中：js文件</p>
<p>●优先级低：图片</p>
<h3 id="服务端推送" tabindex="-1"><a class="header-anchor" href="#服务端推送" aria-hidden="true">#</a> 服务端推送</h3>
...
<h3 id="个人理解" tabindex="-1"><a class="header-anchor" href="#个人理解" aria-hidden="true">#</a> 个人理解</h3>
<p>http2.0安全且高效，安全是因为http2.0是建立在https协议基础上的，高效是因为它采用了二进制分帧来进行数据传输。http2.0新增了二进制分帧层，流是连接中的通道，每个流都有唯一标识符，流标识是描述二进制帧的，与流标识联系的是同一个流，http1.x中的头部放在Headers帧中，请求体放在Data帧中。http2.0的多路复用使得每次传输不需要发起多个请求，把数据流分成互不依赖的帧，最后再在另一端重组。头部压缩主要依靠头部字段表，一开始的头部字段表会放一些常见的，如果有不存在的字段就会在表末尾追加新的http header缓存。将http消息分成很多独立帧之后会存在优先级，优先级从高到低：主要的html，css，js，图片。服务端可以实现在没有请求的时候发送响应，当服务器需要向客户端主动发送响应时，会先发送Frame Type为PUSH_PROMISE的Frame，里面含有PUSH的流ID，客户端解析Frame发现时PUSH_PROMISE时，便会准备接受服务端推送的流</p>
...
<p>数据的完整性未校验，容易被篡改</p>
<p>没有验证对方身份，存在冒充危险</p>
<p>https可以理解为http+ssl/tls，通过SSL证书验证服务器身份，并为浏览器和服务器之间的通信加密</p>
<h3 id="https做了什么" tabindex="-1"><a class="header-anchor" href="#https做了什么" aria-hidden="true">#</a> https做了什么</h3>
<p>HTTPS 协议提供了三个关键的指标</p>
<ul>
<li><code v-pre>加密(Encryption)</code>， HTTPS 通过对数据加密来使其免受窃听者对数据的监听，这就意味着当用户在浏览网站时，没有人能够监听他和网站之间的信息交换，或者跟踪用户的活动，访问记录等，从而窃取用户信息。</li>
<li><code v-pre>数据一致性(Data integrity)</code>，数据在传输的过程中不会被窃听者所修改，用户发送的数据会<code v-pre>完整</code>的传输到服务端，保证用户发的是什么，服务器接收的就是什么。</li>
<li><code v-pre>身份认证(Authentication)</code>，是指确认对方的真实身份，也就是<code v-pre>证明你是你</code>（可以比作人脸识别），它可以防止中间人攻击并建立用户信任。</li>
</ul>
<h2 id="握手阶段" tabindex="-1"><a class="header-anchor" href="#握手阶段" aria-hidden="true">#</a> 握手阶段</h2>
<p>握手阶段</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/2771952479-5e1ad3408e724_fix732.webp" alt=""></p>
<p>非对称加密用在生成这个随机字符串上，属于握手阶段。</p>
<p>对称加密是在真正的加密环节中使用的，key是在握手阶段协商的key</p>
<h3 id="浏览器在使用https传输数据的流程是什么" tabindex="-1"><a class="header-anchor" href="#浏览器在使用https传输数据的流程是什么" aria-hidden="true">#</a> 浏览器在使用HTTPS传输数据的流程是什么</h3>
<ol>
<li>首先客户端通过URL访问服务器建立SSL连接。</li>
<li>服务端收到客户端请求后，会将网站支持的证书信息（证书中包含公钥）传送一份给客户端。</li>
<li>客户端的服务器开始协商SSL连接的安全等级，也就是信息加密的等级。</li>
<li>客户端的浏览器根据双方同意的安全等级，建立会话密钥(session key,是对称加密的)，然后利用网站的公钥将会话密钥加密，并传送给服务器。</li>
<li>服务器利用自己的私钥解密出会话密钥。</li>
<li>服务器利用会话密钥加密与客户端之间的通信。</li>
</ol>
<h3 id="ssl-tls是如何加密的" tabindex="-1"><a class="header-anchor" href="#ssl-tls是如何加密的" aria-hidden="true">#</a> SSL/TLS是如何加密的</h3>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/20220203215335.png" alt=""></p>
<p>RSA (一种非对称加密算法)的运算速度非常慢，而 AES(一种对称加密算法) 的加密速度比较快，而 TLS 正是使用了这种<code v-pre>混合加密</code>方式。在通信刚开始的时候使用非对称算法，比如 RSA、ECDHE ，首先解决<code v-pre>密钥交换</code>的问题。然后用随机数产生对称算法使用的<code v-pre>会话密钥（session key）</code>，再用<code v-pre>公钥加密</code>。对方拿到密文后用<code v-pre>私钥解密</code>，取出会话密钥。这样，双方就实现了对称密钥的安全交换。</p>
<p>加密和解密都是依靠会话密钥的，会话密钥是由公钥加密私钥解密的</p>
<h3 id="https是如何保证数据的完整性的" tabindex="-1"><a class="header-anchor" href="#https是如何保证数据的完整性的" aria-hidden="true">#</a> https是如何保证数据的完整性的</h3>
<p>在 TLS 中，实现完整性的手段主要是 <code v-pre>摘要算法(Digest Algorithm)</code>，MD5 可用于从任意长度的字符串创建 128 位字符串值。MD5 最常用于<code v-pre>验证文件</code>的完整性。</p>
<p>除了常用的 MD5 是加密算法外，<code v-pre>SHA-1(Secure Hash Algorithm 1)</code> 也是一种常用的加密算法。目前 TLS 推荐使用的是 SHA-1 的后继者：<code v-pre>SHA-2</code>。不过 SHA-2 是基于明文的加密方式，还是不够安全，那应该用什么呢？</p>
<p>安全性更高的加密方式是使用 <code v-pre>HMAC</code></p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220203224326950.png" alt="image-20220203224326950"></p>
<p>我们只要在原文后附上它的摘要，就能够保证数据的完整性</p>
<h3 id="https如何进行身份认证" tabindex="-1"><a class="header-anchor" href="#https如何进行身份认证" aria-hidden="true">#</a> https如何进行身份认证</h3>
<p>首先服务器会发来自己的数字证书，这个数字证书是由CA颁发的，这个数字证书内的信息包括：</p>
<p>签发者，证书用途，公钥，加密算法，hash算法，证书到期时间等。</p>
<p>但是如果就这样把证书交给了浏览器，则无法保证证书是否会被人篡改，因此我们需要对以上内容进行一次hash，得到固定长度，再用CA私钥加密该hash，就得到了数字签名，放在证书末尾，任何对证书的篡改操作都会被签名发现。</p>
<p>浏览器拿到证书后，查找操作系统内置的ca公钥，用这个公钥解密数字签名，拿到原始HASHs，再根据证书中hash算法算出一个HASHc，如果两者相等，那么认证成功</p>
<p>https可以在控制台或者抓包到明文吗？可以，控制台属于应用层，加密解密都是在安全层，因此看到的是明文，抓包看到明文实际上是一个代理服务器，并且他有一个证书，客户端信任他的证书。</p>
<h2 id="中间人攻击" tabindex="-1"><a class="header-anchor" href="#中间人攻击" aria-hidden="true">#</a> 中间人攻击</h2>
<p>在握手阶段完成之后，由于有对称加密，传输过程就不会出现问题，但是在握手阶段，客户端没办法知道公钥一定是服务器的，也可能是被别人替换了的。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-c181b1f7cf755605bf950dff0182674a_720w.jpg" alt=""></p>
<p>中间人可以在这个过程中替换掉公钥，这样就可以通过中间人自己这边的私钥解密出premaster（因为客户端的公钥也是中间人给的）并且计算出会话密钥</p>
<h2 id="udp" tabindex="-1"><a class="header-anchor" href="#udp" aria-hidden="true">#</a> UDP</h2>
<p>UDP不提供复杂的控制机制，利用IP提供面向无连接的通信服务。并且它是将应用程序发来的数据在收到的那一刻，立刻按照原样发送到网络上的一种机制。即使出现了网络拥堵，UDP也无法进行流量控制等避免网络拥塞的行为。此外，传输途中即使出现了丢包，UDP也不负责重发。甚至包的顺序乱掉时也没有纠正的功能。如果需要这些细节控制，就需要UDP的应用程序来处理。</p>
<h4 id="udp如何实现可靠传输" tabindex="-1"><a class="header-anchor" href="#udp如何实现可靠传输" aria-hidden="true">#</a> UDP如何实现可靠传输</h4>
...
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-c01a6511abf81f867fd0e531bd51af2c_720w.jpg" alt="img"></p>
<h2 id="tcp" tabindex="-1"><a class="header-anchor" href="#tcp" aria-hidden="true">#</a> TCP</h2>
<p>UDP是一种没有复杂控制，提供面向无连接通信服务的一种协议。换句话说，他将部分控制转移给应用程序去处理，自己却只能提供作为传输层协议的最基本功能。TCP与UDP相差相当大。它充分的实现了数据传输时各种控制功能。它会在丢包时重发，对次序乱掉的包纠正。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-3bd45a13afe868cae6225d75b85e9c36_720w.jpg" alt="img"></p>
<p>数据偏移：其实就是首部长度</p>
<p>控制位包括：CWR，ECE，URG，ACK，PSH，RST，SYN，FIN。</p>
<p>**ACK：**该位设为 1，确认应答的字段有效，TCP规定除了最初建立连接时的 SYN 包之外该位必须设为 1；</p>
<p>**SYN：**用于建立连接，该位设为 1，表示希望建立连接，并在其序列号的字段进行序列号初值设定；</p>
<p>**FIN：**该位设为 1，表示今后不再有数据发送，希望断开连接。当通信结束希望断开连接时，通信双方的主机之间就可以相互交换 FIN 位置为 1 的 TCP 段。</p>
...
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-0673bbc84fe4440aed9d1dadc67ae79b_720w.jpg" alt="img"></p>
...
<li>A 收到 B 的连接确认报文后，还要向 B 发出确认，确认号为 y+1。</li>
</ul>
<p>B 收到 A 的确认后，连接建立。</p>
<h4 id="为什么是三次" tabindex="-1"><a class="header-anchor" href="#为什么是三次" aria-hidden="true">#</a> 为什么是三次</h4>
<p>1、第三次握手是为了防止失效的连接请求到达服务器，让服务器错误打开连接。</p>
<p>如果是两次，假设有一次发送端没有收到接收端的确认报文，那发送端将再一次发送SYN=1的请求报文，这次他收到了并且建立了连接，不久后有断开了了连接，但是如果是只是当初第一次发送的请求报文只是太慢了呢，它在断开连接之后，有一次发送到了接收端，这时如果是两次连接服务端就会打开连接，而客户端则一脸懵逼，因为这时他还是个listen状态，而服务端只需要在listen状态下收到一个请求报文就会打开。</p>
<p>2、双方都能明确自己和对方的收、发能力是正常的。</p>
<p>第一次握手服务端知道了客户端发送能力和服务端接收能力是正常的</p>
<p>第二次握手客户端知道自己发送接受能力和服务端发送接受能力是正常的</p>
<p>第三次握手服务端知道自己发送接收能力和客户端发送接收能力是正常的</p>
<h3 id="四次挥手" tabindex="-1"><a class="header-anchor" href="#四次挥手" aria-hidden="true">#</a> 四次挥手</h3>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-8bf6f231cdd87b2613554f54424e8201_720w.jpg" alt="img"></p>
<p>在tcp连接建立之后，双方的地位是平等的，双方都可以发送数据，也可以接收数据，因此，当客户端收到确认报文之后，服务端还是可以接着发送数据，因为服务端并没有想断开连接，等到服务端想断开连接时，服务端才会发送一个FIN段，客户端收到FIN后在等待2msl(报文最大生存空间)后才会断开连接。</p>
<h4 id="为什么会有2msl" tabindex="-1"><a class="header-anchor" href="#为什么会有2msl" aria-hidden="true">#</a> 为什么会有2msl</h4>
<p>1.如果客户端马上断开了连接，此时都还不知道服务端收没收到ack，如果服务端没有收到ack，超时重传机制服务端就会不断地发送FIN，但客户端也不是不能接受，但是会出现异常，接着服务端就一直不能正常关闭。</p>
<p>2.如果有一些报文段因为网络延时没有送达，如果马上断开连接的话，那么在下次建立连接，或者建立连接后就会就会出现一系列未知的异常，比如说有个syn包，那么下次连接就开始了，在2msl的时间内足以让这些报文段到达。</p>
<h3 id="tcp如何实现可靠传输" tabindex="-1"><a class="header-anchor" href="#tcp如何实现可靠传输" aria-hidden="true">#</a> TCP如何实现可靠传输</h3>
<p>TCP通过校验和，序列号，确认应答，重发控制，连接管理以及窗口控制等机制实现可靠传输。</p>
<ol>
...
<h4 id="通过序列号与确认应答提高可靠性" tabindex="-1"><a class="header-anchor" href="#通过序列号与确认应答提高可靠性" aria-hidden="true">#</a> 通过序列号与确认应答提高可靠性</h4>
<p>在TCP中，发送端数据到达接收端主机值，接收端主机会返回一个已收到消息的通知，这个消息叫做确认应答（ACK).由于有时候确认应答会丢包，因此我们引入了序列号，就是按顺序给发送数据的每一个字节都标上号码，在确认应答的时候就发送确认应答号来告诉发送端下一次要发什么（虽然我至今还是没有搞明白这个序列号怎么解决的丢包的问题的）(回来解决：如果接收端返回了带有这个序列号的ack包，那么这个就代表这个包已经收到了，如果没有这个序列号的话是不知道哪个包有没有丢了，毕竟发送数据并不会等待上一个报确认后再发，而是发好几个，发多少的话是根据双方的接受能力定的)</p>
<h4 id="滑动窗口" tabindex="-1"><a class="header-anchor" href="#滑动窗口" aria-hidden="true">#</a> 滑动窗口</h4>
<p>是流量控制的手段，我们不再是发送端等到接收端发回确认应答再接着发，而是依靠一个滑动窗口，每次发送端发送完数据后将这部分数据加入到缓冲区中，并且接着发送下面的数据，当收到确认请求后清空缓冲区中这块数据。</p>
<h4 id="窗口控制与重发控制" tabindex="-1"><a class="header-anchor" href="#窗口控制与重发控制" aria-hidden="true">#</a> 窗口控制与重发控制</h4>
<p>有了窗口控制后，发送端不会因为没有收到确认应答而重发，因为有可能会是接收端收到了但是确认应答丢失了，因此如果是某一个序列号的包丢失了，接收端会在每次收到数据时发送同一个序列号的确认应答，发送端收到三次相同的确认应答就会重发<img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220410102502702.png" alt="image-20220410102502702"></p>
<p>在某一报文段丢失后，发送端会一直收到序号为（1001）的确认应答，告诉发送端1001这块报文段丢失了，如果发送端收到了三次同一个确认应答就会重发。</p>
<h4 id="拥塞控制" tabindex="-1"><a class="header-anchor" href="#拥塞控制" aria-hidden="true">#</a> 拥塞控制</h4>
<p>在通信刚开始的时候就发送大量数据会引发其他问题，TCP为了防止该问题，在通信一开始就会通过一个叫慢启动的算法得出的数值，对发送数据量进行控制，为了在发送端调节所要发的数据量，定义了一个叫‘拥塞窗口’的概念。**慢启动：**一开始拥塞窗口会设置成1MSS，每次收到确认应答都会加1，拥塞窗口就会以1，2，4指数式增长，为了防止这些引入了慢启动阈值。<strong>拥塞避免</strong>：只要拥塞窗口的值超过了慢启动阈值，只允许以1个数据段的字节数*1个数据段的字节数/拥塞窗口字节数的比例增大。**快恢复：**TCP的通信一开始并没有设置阈值，而是在超时重发时，才会设置为当前拥塞窗口的一半，重复确认应答进行高速重发控制，也会将阈值设为拥塞窗口的一半，接着会把拥塞窗口设置成慢启动阈值+3个数据段大小，这样TCP的吞吐量就可以随着网络的拥塞情况发生改变。</p>
<h4 id="快重传、快恢复" tabindex="-1"><a class="header-anchor" href="#快重传、快恢复" aria-hidden="true">#</a> 快重传、快恢复</h4>
<p>快重传算法规定，发送方只要一连收到三个重复确认就应当立即重传对方尚未收到的报文段，而不必继续等待设置的重传计时器时间到期。</p>
<p><strong>快恢复：<strong>当发送方连续收到三个重复确认时，就执行</strong>乘法减小</strong>算法，把拥塞窗口阈值减半（为了预防网络发生拥塞）。但是接下去并不执行慢开始算法，而是将当前发送的窗口设置为拥塞窗口阈值减半后的值，然后执行拥塞避免算法。</p>
<h1 id="ip协议" tabindex="-1"><a class="header-anchor" href="#ip协议" aria-hidden="true">#</a> IP协议</h1>
<p>IP相当于是OSI参考模型的第三层-----网络层</p>
<p>在之前的数据链路层，我们可以通过交换机，可以将两个数据链路连接起来，但是这只局限于同一种数据链路，如果要将不同种数据链路(以太网、无线LAN亦或是PPP)连接起来的话，就需要借助网络层(中的路由器)，路由器是网络层的核心人物，配置有ip地址但没有路由控制的设备称为主机，配置有ip地址并且同时又路由控制的设备称为路由器，节点是路由器和主机的总称。MAC地址是用来标识同一链路不同计算机的，IP地址用于在连接到网络中的所有主机中识别出进行通信的目标地址。因此想要发送数据到任意IP地址，需要如下流程（ip层面）：首先发送方会通过子网掩码来判断目标ip地址与自己是否在同一局域网内，如果不在就会发给默认网关(即路由器的ip地址)，路由器有一张路由控制表，路由控制表会将端口与下一跳的子网对应起来，接着只需要传给对应端口就好了。当然数据链路层是不知道IP地址的，他们只知道mac地址，那么就需要一张mac与ip地址对应的表，这就叫arp缓存表。比如说我想要将跳到默认网关，默认网关是一个ip地址，这时候就需要arp表，通过arp表得到mac地址，通过交换机或者其他交换设备，就可以把数据包发送给默认网关。</p>
<h2 id="ipv6" tabindex="-1"><a class="header-anchor" href="#ipv6" aria-hidden="true">#</a> IPv6</h2>
<h3 id="ipv6的必要性" tabindex="-1"><a class="header-anchor" href="#ipv6的必要性" aria-hidden="true">#</a> IPv6的必要性</h3>
<p>IPv6是为了根本解决IPv4地址耗尽的问题而被标准化的网际协议，IPv4的地址长度为32比特，而IPv6地址长度是128比特，IPv6不仅仅能解决IPv4地址耗尽的问题它甚至试图弥补IPv4绝大多数缺陷。</p>
<ul>
<li>IP地址的扩大和路由控制表的聚合</li>
<li>性能提升</li>
<li>采用认证和加密功能，应对伪造IP地址的网络安全功能以及防止线路窃听的功能</li>
</ul>
<h1 id="dns" tabindex="-1"><a class="header-anchor" href="#dns" aria-hidden="true">#</a> dns</h1>
...
<p>5.如果本地DNS服务器本地区域文件与缓存解析都失效，则根据本地DNS服务器的设置（是否设置转发器）进行查询，如果未用转发模式，本地DNS就把请求发至13台根DNS，根DNS服务器收到请求后会判断这个域名(.com)是谁来授权管理，并会返回一个负责该顶级域名服务器的一个IP。本地DNS服务器收到IP信息后，将会联系负责.com域的这台服务器。这台负责.com域的服务器收到请求后，如果自己无法解析，它就会找一个管理.com域的下一级DNS服务器地址http://qq.com给本地DNS服务器。当本地DNS服务器收到这个地址后，就会找http://qq.com域服务器，重复上面的动作，进行查询，直至找到www . qq .com主机。</p>
<p>6.如果用的是转发模式，此DNS服务器就会把请求转发至上一级DNS服务器，由上一级服务器进行解析，上一级服务器如果不能解析，或找根DNS或把转请求转至上上级，以此循环。不管是本地DNS服务器用是是转发，还是根提示，最后都是把结果返回给本地DNS服务器，由此DNS服务器再返回给客户机。
从客户端到本地DNS服务器是属于递归查询，而DNS服务器之间就是的交互查询就是迭代查询。</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/464291-20170703113844956-354755333.jpg" alt="img"></p>
<p>在通过<strong>浏览器缓存</strong>（为加快访问速度，有些浏览器会缓存dns记录）及<strong>host文件</strong>都无法解析域名的情况下，OS会将这个域名发送给计算机网络配置中DNS对应的地址（LDNS），即本地区的域名服务器。这个DNS通常都提供给你本地互联网接入的一个DNS解析服务，假如是在学校接入的互联网，那么这个本地区的域名服务器基本上是在学校中；如果是在小区接入的互联网，那么这个本地区的域名服务器就是提供给你接入互联网的应用服务上，也就是电信或联通。</p>
<h1 id="cdn" tabindex="-1"><a class="header-anchor" href="#cdn" aria-hidden="true">#</a> cdn</h1>
<p>CDN 的工作原理就是将您源站的资源缓存到位于全球各地的 CDN 节点上，用户请求资源时，就近返回节点上缓存的资源，而不需要每个用户的请求都回您的源站获取，避免网络拥塞、缓解源站压力，保证用户访问资源的速度和体验。</p>
<ol>
...
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/v2-5793aec83fc645e002a1cd70ab7209a3_720w.jpg" alt="img"></p>
<p>使用A记录和CNAME进行域名解析的区别</p>
<p>A记录就是把一个域名解析到一个IP地址（Address，特制数字IP地址），而                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        CNAME记录就是把域名解析到另外一个域名。其功能是差不多，CNAME将几个主机名指向一个别名，其实跟指向IP地址是一样的，因为这个别名也要做一个A记录的。</p>
<h1 id="原型链" tabindex="-1"><a class="header-anchor" href="#原型链" aria-hidden="true">#</a> 原型链</h1>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/原型链.jpg" alt="image">
Foo和Object都是函数，函数都是由Function构造出来的，Function是函数的构造函数，但是不常用，函数独有prototype,指向其原型对象，任何对象都有_proto_属性，指向构造该对象的构造函数的原型。图中Foo.prototype和Function.prototype都是一个对象，他们的_proto_(隐式原型)指向Object.prototype</p>
<h1 id="flex" tabindex="-1"><a class="header-anchor" href="#flex" aria-hidden="true">#</a> flex</h1>
<p>flex:1 是指 flex:1 1 auto
...
flex-basis:给上面两个属性分配多余空间之前, 计算项目是否有多余空间, 默认值为 auto, 即项目本身的大小</p>
<p>子元素的原始宽度是由<code v-pre>flex-basis</code>来决定的，<code v-pre>flex-basis:auto</code>表示子元素原始宽度为子元素自身的宽度。</p>
<p><code v-pre>flex-grow</code> 表示如果容器还剩有宽度，那么剩余空间将会按照flex-grow的比例来分配</p>
<p>同理<code v-pre>flex-shrink</code> 表示如果所有项的原始宽度大于容器宽度，那么元素缩小比例也会按照<code v-pre>flex-shrink</code>来分配</p>
<p>flex:xxx如果去百分比或者长度(px)则表示flex-grow:1 flex-shrink:1</p>
<p>所有 flex 元素都会有下列行为：</p>
<ul>
...
<p>因为交叉轴会被拉伸来填充交叉轴大小，前提是元素高度或者宽度没确定</p>
<p>order属性用来对元素排序</p>
<p>不设置默认是0</p>
<p>数字越小排在月前面</p>
<h1 id="bfc-块级格式上下文" tabindex="-1"><a class="header-anchor" href="#bfc-块级格式上下文" aria-hidden="true">#</a> BFC(块级格式上下文)</h1>
...
<h2 id="bfc特性" tabindex="-1"><a class="header-anchor" href="#bfc特性" aria-hidden="true">#</a> BFC特性</h2>
<p>1.内部的Box会在垂直方向上一个接一个的放置。
2.垂直方向上的距离由margin决定
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>container<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>p</span><span class="token punctuation">></span></span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>p</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>div</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>把两个盒子放在bfc容器中就可以解决外边距折叠的问题。</p>
<p><strong>清除浮动</strong></p>
<p><strong>不被浮动元素覆盖</strong></p>
<p>二栏布局，左边宽度固定，右边宽度自适应，把右边设置为bfc就可以实现</p>
<p>还可以防止字体环绕</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>和上面的方法差不多</p>
<p><strong>利用overflow</strong></p>
<p>其本质使用bfc来实现</p>
<h1 id="react" tabindex="-1"><a class="header-anchor" href="#react" aria-hidden="true">#</a> React</h1>
<h2 id="react中方法this指向问题" tabindex="-1"><a class="header-anchor" href="#react中方法this指向问题" aria-hidden="true">#</a> react中方法this指向问题</h2>
<p>在react中类式组件中的方法会放在该类的原型对象上，供实例使用
类中的方法默认开启局部严格模式，this指向undefined</p>
<h2 id="受控组件" tabindex="-1"><a class="header-anchor" href="#受控组件" aria-hidden="true">#</a> 受控组件</h2>
<p>在HTML表单元素中，它们通常自己维护一套<code v-pre>state</code>,并随着用户的输入改变UI，如果我们把react的state和表单元素建立依赖关系，再通过<code v-pre>onChange</code>和<code v-pre>setState</code>结合更新state属性，就能达到控制用户输入过程中表单发生的操作。</p>
<h1 id="事件循环机制" tabindex="-1"><a class="header-anchor" href="#事件循环机制" aria-hidden="true">#</a> 事件循环机制</h1>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/事件循环1.webp" alt="image">
同步和异步任务分别进入不同的执行&quot;场所&quot;，同步的进入主线程，异步的进入Event Table并注册函数。</p>
<p>当指定的事情完成时，Event Table会将这个函数移入Event Queue。</p>
<p>主线程内的任务执行完毕为空，会去Event Queue读取对应的函数，进入主线程执行。</p>
<p>上述过程会不断重复，也就是常说的Event Loop(事件循环)。
<img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/事件循环2.webp" alt="image">
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最终结果:1，7，6，8，2，4，3，5，9，11，10，12</p>
<h1 id="跨域" tabindex="-1"><a class="header-anchor" href="#跨域" aria-hidden="true">#</a> 跨域</h1>
<p>有一个小小的东西叫cookie大家应该知道，一般用来处理登录等场景，目的是让服务端知道谁发出的这次请求。如果你请求了接口进行登录，服务端验证通过后会在响应头加入Set-Cookie字段，然后下次再发请求的时候，浏览器会自动将cookie附加在HTTP请求的头字段Cookie中，服务端就能知道这个用户已经登录过了。知道这个之后，我们来看场景：
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a href="https://www.cnblogs.com/leegent/p/7499532.html" target="_blank" rel="noopener noreferrer">https://www.cnblogs.com/leegent/p/7499532.html<ExternalLinkIcon/></a>
2.空iframe加form
...
<p>1)，请求的方法是get，head或者post，同时Content-Type是application/x-www-form-urlencoded, multipart/form-data 或 text/plain中的一个值，或者不设置也可以，一般默认就是application/x-www-form-urlencoded。</p>
<p>2)，请求中没有自定义的HTTP头部，如x-token。(应该是这几种头部 Accept，Accept-Language，Content-Language，Last-Event-ID，Content-Type）</p>
<p>浏览器：把客户端脚本所在的域填充到Origin header里，向其他域的服务器请求资源。</p>
<p>服务器：根据资源权限配置，在响应头中添加Access-Control-Allow-Origin Header，返回结果</p>
<p>浏览器：观察服务器是否有返回Access-Control-Allow-Origin Header以及Access-Control-Allow-Origin Header是否与origin相同,如果是，则接受这个内容。否则浏览器忽略此次响应。
二：带预检(Preflighted)的跨域请求，流程如下</p>
<p>网页：当HTTP请求出现以下两种情况时之一，浏览器认为是带预检(Preflighted)的跨域请求：</p>
<p>1），请求的方法不是 GET, HEAD或者POST三种，或者是这三种，但是Content-Type不是application/x-www-form-urlencoded, multipart/form-data或text/plain中的一种。</p>
<p>2），请求中有自定义HTTP头部。</p>
<p>浏览器：发现请求属于以上两种情况，向服务器发送一个OPTIONS预检请求，检测服务器端是否支持真实请求进行跨域资源访问，浏览器会在发送OPTIONS请求时会自动添加Origin Header 、Access-Control-Request-Method Header(将实际请求的方法告诉服务器)和Access-Control-Request-Headers Header。</p>
<p>服务器：响应OPTIONS请求，会在responseHeader里添加Access-Control-Allow-Methods head。这其中的method的值是服务器给的默认值，可能不同的服务器添加的值不一样。服务器还会添加Access-Control-Allow-Origin Header和Access-Control-Allow-Headers Header。这些取决于服务器对OPTIONS请求具体如何做出响应。如果服务器对OPTIONS响应不合你的要求，你可以手动在服务器配置OPTIONS响应，以应对带预检的跨域请求。在配置服务器OPTIONS的响应时，可以添加Access-Control-Max-Age head告诉浏览器在一定时间内无需再次发送预检请求，但是如果浏览器禁用缓存则无效。</p>
<p>浏览器：接到OPTIONS的响应，先判断状态码是不是200，比较真实请求的method是否属于返回的Access-Control-Allow-Methods head的值之一，还有origin, head也会进行比较是否匹配。如果通过，浏览器就继续向服务器发送真实请求。 否则就会报预检错误，如下几种：</p>
...
<p>个人理解:简单请求的话请求的方法和请求头都是已经满足要求了的，所以服务器只需要对请求域进行设置。带预检的跨域请求由于请求方法和请求头都没有被限制过因此需要先发送options请求告诉服务器我们实际发送请求的时候会使用到什么方法，和header，接着服务器就会在响应头中告诉我们他们允许 什么方法 什么域 什么头。
<img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/cors1.webp" alt="image">
<img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/cors2.webp" alt="image"></p>
<h2 id="nginx反向代理" tabindex="-1"><a class="header-anchor" href="#nginx反向代理" aria-hidden="true">#</a> nginx反向代理</h2>
<h1 id="正向代理和反向代理" tabindex="-1"><a class="header-anchor" href="#正向代理和反向代理" aria-hidden="true">#</a> 正向代理和反向代理</h1>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>能这么做的原理又是另外一道经典面试题：<code v-pre>new操作符都做了什么</code>，很简单，就<code v-pre>4</code>点：</p>
<p>1.创建一个空对象</p>
<p>2.把该对象的<code v-pre>__proto__</code>属性指向<code v-pre>Sub.prototype</code></p>
<p>3.让构造函数里的<code v-pre>this</code>指向新对象，然后执行构造函数，</p>
<p>4.返回该对象</p>
<p>所以<code v-pre>People.call(this)</code>的<code v-pre>this</code>指的就是这个新创建的对象，那么就会把父类的实例属性/方法都添加到该对象上。</p>
<p><strong>特点</strong>：</p>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="es5和es6继承的区别" tabindex="-1"><a class="header-anchor" href="#es5和es6继承的区别" aria-hidden="true">#</a> ES5和ES6继承的区别</h2>
<p>**区别1：**ES6里的构造函数就是一个普通的函数，可以使用new调用，也可以直接调用，而ES6的class不能当作普通函数调用，必须使用new操作符调用。</p>
<p><strong>区别2：</strong><code v-pre>ES5</code>的原型方法和静态方法默认是可枚举的，而class的默认不可枚举，如果想要获取不可枚举的属性可以使用<code v-pre>Object.getOwnPropertyNames</code>方法</p>
<p>**区别3：**子类可以直接通过<code v-pre>__proto__</code>找到父类，而ES5是指向<code v-pre>Function.prototype</code>：</p>
<p>ES6：<code v-pre>Sub.__proto__ === Sup</code></p>
<p>ES5：<code v-pre>Sub.__proto__ === Function.prototype</code></p>
<p>**区别4：**ES5的继承，实质是先创造子类的实例对象<code v-pre>this</code>，然后再执行父类的构造函数给它添加实例方法和属性(不执行也无所谓）。而ES6的继承机制完全不同，实质是先创造父类的实例对象<code v-pre>this</code>（当然它的<code v-pre>__proto__</code>指向的是子类的<code v-pre>prototype</code>），然后再用子类的构造函数修改<code v-pre>this</code>。</p>
<p>这就是为啥使用<code v-pre>class</code>继承在<code v-pre>constructor</code>函数里必须调用<code v-pre>super</code>，因为子类压根没有自己的<code v-pre>this</code>，另外不能在<code v-pre>super</code>执行前访问<code v-pre>this</code>的原因也很明显了，因为调用了<code v-pre>super</code>后，<code v-pre>this</code>才有值。</p>
<p>**区别5：**class不存在变量提升，所以父类必须在子类之前定义</p>
...
Object<span class="token punctuation">.</span><span class="token function">defineProperty</span><span class="token punctuation">(</span>Person<span class="token punctuation">,</span> <span class="token string">'name'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>get:给属性提供getter方法</p>
<p>set：给属性提供setter方法</p>
<h2 id="数据劫持" tabindex="-1"><a class="header-anchor" href="#数据劫持" aria-hidden="true">#</a> 数据劫持</h2>
<p>vue.js采用<strong>数据劫持结合发布者-订阅者模式</strong>的方式，通过<strong>Object.defineProperty()来劫持各个属性的setter和getter，在数据变动时发布消息给订阅者，触发相应的监听回调</strong>。</p>
<h1 id="严格模式" tabindex="-1"><a class="header-anchor" href="#严格模式" aria-hidden="true">#</a> 严格模式</h1>
<p>严格模式的变化通常分为这几类：将问题直接转化为错误（如语法错误或运行时错误）, 简化了如何为给定名称的特定变量计算，简化了 eval 以及 arguments, 将写&quot;安全“JavaScript 的步骤变得更简单，以及改变了预测未来 ECMAScript 行为的方式。</p>
<h2 id="_1-1将过失转化为异常" tabindex="-1"><a class="header-anchor" href="#_1-1将过失转化为异常" aria-hidden="true">#</a> 1.1将过失转化为异常</h2>
<ul>
...
<li>严格模式要求函数的参数名唯一（严格模式要求函数的参数名唯一）</li>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_1-2简化变量的使用" tabindex="-1"><a class="header-anchor" href="#_1-2简化变量的使用" aria-hidden="true">#</a> 1.2简化变量的使用</h2>
<ul>
<li>
<p>严格模式禁用 with</p>
...
var x;
delete x; // !!! 语法错误
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_1-3让eval和arguments变得简单" tabindex="-1"><a class="header-anchor" href="#_1-3让eval和arguments变得简单" aria-hidden="true">#</a> 1.3让eval和arguments变得简单</h2>
<ul>
...
<h2 id="_1-4-安全的javascript" tabindex="-1"><a class="header-anchor" href="#_1-4-安全的javascript" aria-hidden="true">#</a> 1.4 安全的javascript</h2>
<ul>
<li>
<p>在严格模式下通过 this 传递给一个函数的值不会被强制转换为一个对象。</p>
</li>
<li>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_1-5-为未来的-ecmascript-版本铺平道路" tabindex="-1"><a class="header-anchor" href="#_1-5-为未来的-ecmascript-版本铺平道路" aria-hidden="true">#</a> 1.5 为未来的 ECMAScript 版本铺平道路</h2>
<ul>
<li>
<p>在严格模式中一部分字符变成了保留的关键字。</p>
</li>
<li>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="网络安全" tabindex="-1"><a class="header-anchor" href="#网络安全" aria-hidden="true">#</a> 网络安全</h1>
...
<p>水平方向：水平方向上，都有效；</p>
<p>垂直方向：垂直方向上，都无效；（padding-top 和 padding-bottom 会显 示出效果，但是高度不会撑开，不会对周围元素有影响）</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>-webkit-box-orient 和-webkit-line-clamp是webkit是chrome、safari私有属性，webkit是用来浏览器绘制网页的排版引擎，因此需要display:-webkit-box;</p>
<h2 id="css3新特性" tabindex="-1"><a class="header-anchor" href="#css3新特性" aria-hidden="true">#</a> CSS3新特性</h2>
<ul>
<li>border-radius</li>
<li>box-shadow</li>
<li>border-image</li>
<li>background-clip、background-origin、background-size和background-break</li>
<li>word-wrap</li>
<li>text-overflow</li>
...
<li>transition</li>
<li>animation</li>
<li>linear-gradient、linear-gradient</li>
<li>flex、grid</li>
</ul>
...
<h2 id="伪类伪元素" tabindex="-1"><a class="header-anchor" href="#伪类伪元素" aria-hidden="true">#</a> 伪类伪元素</h2>
<p>1、伪类是类，所以跟css选择器有关，不存在与dom中
2、CSS伪类是用来添加一些选择器的特殊效果的
3、伪类前面之后一个冒号**:first-child**</p>
<p>伪元素是创造文档树之外的对象。伪元素也是元素，只不过不存在与dom对象中，但是浏览器审查元素的时候能看得到的，比如::before ::after</p>
<h2 id="position-sticky" tabindex="-1"><a class="header-anchor" href="#position-sticky" aria-hidden="true">#</a> position:sticky</h2>
<p>sticky属性是relative和fixed的结合，你可以设置top，bottom，left，right实现粘滞的效果，sticky的任意父元素必须是overflow:visible,如果是overflow:hidden，那么该父元素无法滚动。sticky会相对上一个定位过的父元素进行定位</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="前端性能优化" tabindex="-1"><a class="header-anchor" href="#前端性能优化" aria-hidden="true">#</a> 前端性能优化</h1>
<ul>
<li>减少http请求</li>
<li>静态资源使用cdn</li>
<li>css文件放在头部，js文件放在底部</li>
<li>使用iconfont代替图片图标</li>
<li>善用缓存</li>
<li>用插件压缩文件(webpack)</li>
<li>图片懒加载(图片出现在可视区域再加载)</li>
<li>降低图片质量(使用插件压缩)</li>
<li>尽量使用css3效果代替图片</li>
<li>使用webp格式的图片</li>
<li>通过webpack按需加载代码</li>
<li>减少回流重绘</li>
<li>使用事件委托</li>
<li>使用flex布局而不是较早的布局模型</li>
</ul>
<h1 id="js" tabindex="-1"><a class="header-anchor" href="#js" aria-hidden="true">#</a> JS</h1>
<h2 id="async-await-如何通过同步的方式实现异步" tabindex="-1"><a class="header-anchor" href="#async-await-如何通过同步的方式实现异步" aria-hidden="true">#</a> Async/await 如何通过同步的方式实现异步</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2.加法</strong></p>
<p>由于<code v-pre>+</code>可以用来拼接字符串，因此加法比较特殊。</p>
<ul>
<li>
<p>当一侧为<code v-pre>String</code> 类型时，被识别为字符串拼接，并会优先将另一侧转换为字符串类型。</p>
</li>
<li>
<p>当一侧为<code v-pre>Number</code>类型，另一侧为原始类型，则将原始类型转换为<code v-pre>Number</code>类型。</p>
</li>
</ul>
<ul>
<li>当一侧为<code v-pre>Number</code>类型，另一侧为引用类型，将引用类型和<code v-pre>Number</code>类型转换成字符串后拼接。</li>
</ul>
<h3 id="逻辑语句中的类型转换" tabindex="-1"><a class="header-anchor" href="#逻辑语句中的类型转换" aria-hidden="true">#</a> 逻辑语句中的类型转换</h3>
<p>使用<code v-pre>if while for</code>时，我们希望里面是个<code v-pre>Bollean</code>。</p>
<p><strong>1.单个变量</strong></p>
<p>如果是单个变量时，会将其转换为<code v-pre>Bollean</code></p>
<p>**规则：**只有 <code v-pre>null</code> <code v-pre>undefined</code> <code v-pre>''</code> <code v-pre>NaN</code> <code v-pre>0</code> <code v-pre>false</code> 这几个是 <code v-pre>false</code>，其他的情况都是 <code v-pre>true</code>，比如 <code v-pre>{}</code> , <code v-pre>[]</code>。</p>
<p><strong>2.使用==比较中的五条规则</strong></p>
<ul>
<li>
<p><code v-pre>NAN</code> 和其他类型比较永远返回false(包括自己)</p>
</li>
<li>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>map的另一种创建方式：</strong> <code v-pre>new Map(iterable)</code> 传入的是一个可迭代对象，并且每个值是[key,value],例如：</p>
<p><code v-pre>[['1','str1'],[1,'num1'],[true,'bool1']]</code></p>
<h4 id="map与obj转换" tabindex="-1"><a class="header-anchor" href="#map与obj转换" aria-hidden="true">#</a> map与obj转换</h4>
<ul>
<li>从对象转为 Map ：<code v-pre>new Map(Object.entries(obj))</code></li>
<li>从 Map 转为对象：<code v-pre>Object.fromEntries(map)</code></li>
</ul>
...
<p>与map类似，<code v-pre>set.keys()</code>与<code v-pre>set.values()</code>相同返回的都是value值，<code v-pre>set.entries()</code> 每个值都是[value, value]，为了与map对齐。</p>
...
// john 被从内存中删除了！
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>weakMap不支持迭代以及 <code v-pre>keys()</code>，<code v-pre>values()</code> 和 <code v-pre>entries()</code> 方法。</p>
<h3 id="weakset" tabindex="-1"><a class="header-anchor" href="#weakset" aria-hidden="true">#</a> weakSet</h3>
<p>只能用来存储对象，对象不计入垃圾回收的引入</p>
<p>不支持<code v-pre>size()</code>和<code v-pre>keys()</code>,并不支持迭代</p>
<h3 id="for-of" tabindex="-1"><a class="header-anchor" href="#for-of" aria-hidden="true">#</a> for of</h3>
<p>为什么需要for of
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="es6" tabindex="-1"><a class="header-anchor" href="#es6" aria-hidden="true">#</a> es6</h1>
<p>let const 解构 模板字符串 扩展运算符 for of Object.is Object.assign</p>
<p>字符串:str.repeat() str.startsWith() str.endsWith() includes()</p>
...
<p>let 和 const 就不会再词法环境中加上undefined，因此会出现语法错误(还有class)</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在for循环中，可以理解成使用let会在每次循环时创建一个块级作用域，而使用var变量会放在全局作用域或者函数作用域中，因此调用回调使用let的就会找到自己块级作用域下的正确的i，而var会找到全局或者函数作用域下的i(10)</p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220305214516815.png" alt="image-20220305214516815"></p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220305214543717.png" alt="image-20220305214543717"></p>
<p><img src="https://lzc-personal-resource.oss-cn-beijing.aliyuncs.com/images/typora/image-20220305214822677.png" alt="image-20220305214822677"></p>
<h1 id="hash路由和history路由" tabindex="-1"><a class="header-anchor" href="#hash路由和history路由" aria-hidden="true">#</a> hash路由和history路由</h1>
<h2 id="hash路由" tabindex="-1"><a class="header-anchor" href="#hash路由" aria-hidden="true">#</a> hash路由</h2>
<p>hash路由就是#号后面跟的hash值，通过hashChange事件就可以监听路由变化，从而实现不同路由对应不同组件的效果</p>
<p>优点：</p>
<p>1.只需要前端配置路由表, 不需要后端的参与</p>
<p>2.兼容性好, 浏览器都能支持</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过监听hashchange加载不同的组件</p>
<h2 id="history路由" tabindex="-1"><a class="header-anchor" href="#history路由" aria-hidden="true">#</a> history路由</h2>
<p><img src="https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4b0d880981714fe080f4e2bb678e32ab~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp" alt="img"></p>
<p>我们使用history路由访问某个网站，按照常理说应该是访问服务器的某个资源，如我们访问user/123435 ,如果服务器没有这个资源就会报404，因此使用history路由是需要后台配置的，在访问不存在域名的时候，就会重定向到index.html上。</p>
<p>history是依靠history.pushState(replaceState)完成URL跳转但是不重新加载页面(不发送请求)</p>
<h1 id="箭头函数" tabindex="-1"><a class="header-anchor" href="#箭头函数" aria-hidden="true">#</a> 箭头函数</h1>
<ul>
<li>箭头函数this在定义时绑定，它只会从自己的作用域链的上一层继承this</li>
<li>.call()/.apply()/.bind()无法改变this指向</li>
<li>不能作为构造函数，因为new 的本质就是改变this的指向</li>
<li>箭头函数没有自己的arguments，在箭头函数访问arguments实际上获得的是外层局部(函数)执行环境中的值</li>
<li>箭头函数没有原型</li>
<li>箭头函数不能用作Generator函数，不能使用yeild关键字</li>
</ul>
<h1 id="flux" tabindex="-1"><a class="header-anchor" href="#flux" aria-hidden="true">#</a> flux</h1>
<p>Flux 不是一个框架（Framework）或库（Library），而是一种架构（Architecture）</p>
<ul>
...
<p><a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Number" target="_blank" rel="noopener noreferrer"><code v-pre>Number</code><ExternalLinkIcon/></a> 和 <code v-pre>BigInt</code> 可以进行比较。两者也可以混在一个数组内并排序。</p>
</div></template>
